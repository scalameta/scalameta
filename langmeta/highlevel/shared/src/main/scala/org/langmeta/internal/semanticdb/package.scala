...
import org.langmeta.highlevel.inputs.{Input => dInput}
...
import org.langmeta.highlevel.{semanticdb => d}
        ...
        case d.Document(dinput, dlanguage, dnames, dmessages, dsymbols, dsynthetics) =>
          object dPosition {
            def unapply(dpos: dPosition): Option[s.Position] = dpos match {
              case org.langmeta.highlevel.inputs.Position.Range(_, sstart, send) =>
          ...
          object dDenotation {
            def unapply(ddefn: d.Denotation): Option[s.Denotation] = {
              ...
              val snames = ddefn.names.map {
                case d.ResolvedName(org.langmeta.highlevel.inputs.Position.Range(_, sstart, send), ssym, sisDefinition) =>
