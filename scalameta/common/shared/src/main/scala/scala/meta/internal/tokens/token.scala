...
class TokenNamerMacros(val c: Context) extends TokenNamerMacroHelpers {
  ...
  private def impl(annottees: Seq[Tree], isFixed: Boolean): Tree =
    annottees.transformAnnottees(new ImplTransformer {
      override def transformClass(cdef: ClassDef, mdef: ModuleDef): List[ImplDef] = {
        ...
        val providedTokenName = argss match {
          ...
          case _ =>
            c.abort(c.enclosingPosition, "@token annotation takes a literal string argument")
        }
        ...
        def hasMethod(name: String): Boolean = stats1.exists {
          case DefDef(_, TermName(`name`), _, _, _, _) => true; case _ => false
        }
