...
class DataMacros(val c: Context) extends MacroHelpers {
  ...
  def data(annottees: Tree*): Tree = annottees.transformAnnottees {
    new ImplTransformer {
      override def transformClass(cdef: ClassDef, mdef: ModuleDef): List[ImplDef] = {
        ...
        if (paramss.isEmpty) c
          .abort(cdef.pos, "@data classes must define a non-empty parameter list")

        // step 2: create parameters, generate getters and validation checks
        val paramss1 = paramss.map {
          _.map {
