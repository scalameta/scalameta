...
object PathIO {
  ...
  def workingDirectory: AbsolutePath = AbsolutePath.workingDirectory
  ...
  def dirname(abspath: String): String = {
    ...
    val end = if (isDir) abspath.lastIndexOf('/', abspath.length - 2) else abspath.lastIndexOf('/')
    if (end < 0) "/" else abspath.substring(0, end + 1)
  }
  ...
  def basename(abspath: String): String = {
    ...
    if (end < 0) abspath
      ...
      if (start < 0) abspath.substring(0, end) else abspath.substring(start + 1, end)
    }
  }
