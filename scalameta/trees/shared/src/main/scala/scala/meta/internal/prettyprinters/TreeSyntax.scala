  ...
  def apply[T <: Tree](dialect: Dialect): Syntax[T] = {
    object syntaxInstances {
      ...
      implicit def syntaxTree[T <: Tree]: Syntax[T] = Syntax {
        ...
        case Mod.ProtectedThis()                   => s(kw("protected"), kw("["), kw("this"), kw("]"))
        case Mod.ProtectedWithin(Name.Anonymous()) => s(kw("protected"))
        case Mod.ProtectedWithin(name)             => s(kw("protected"), kw("["), s(name), kw("]"))
