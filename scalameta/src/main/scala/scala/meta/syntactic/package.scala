...
package object syntactic {
  ...
  @quasiquote[Term.Param]('param)      implicit class ParamQuote(ctx: StringContext)
  ...
  @quasiquote[Enumerator]('enumerator) implicit class EnumQuote(ctx: StringContext)
  ...
  object Parse {
    ...
    implicit def parseParam(implicit dialect: Dialect): Parse[Term.Param] = apply(origin => new Parser(origin).parseParam())
