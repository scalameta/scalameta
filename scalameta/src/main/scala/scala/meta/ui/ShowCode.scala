...
object Code {
  ...
  implicit def codeTree[T <: api.Tree](implicit dialect: Dialect, style: Style): Code[T] = Code { x => (x: api.Tree) match {
    // Bottom
    case t: Quasi.Ellipsis       => s("." * (t.rank + 1), a("{", t.tree, "}", !t.tree.isInstanceOf[Quasi.Unquote]))
    case t: Quasi.Unquote        => s("${", t.tree.toString, " @ ", t.pt.getName.stripPrefix("scala.meta.").stripPrefix("internal.ast."), "}")
