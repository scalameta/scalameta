...
package object tasty {
  implicit class XtensionTastyWriteTree(source: SemanticSource) {
  ...
  implicit class XtensionTastyReadTree(dummy: Source.type) {
    def fromTasty(tastyBlob: Array[Byte]): Option[(SyntacticDigest, SemanticSource)] = {
      ...
      for {
        syntacticDigest <- unpickler.unpickle(new SyntacticUnpickler)
        semanticSource <- unpickler.unpickle(new SemanticUnpickler)
      } yield {
