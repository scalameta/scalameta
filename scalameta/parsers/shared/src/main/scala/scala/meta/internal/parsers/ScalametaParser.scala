...
class ScalametaParser(input: Input, dialect: Dialect) { parser =>
  ...
  lazy val (parserTokens, parserTokenPositions) = {
    ...
    @tailrec def loop(prevPos: Int, currPos: Int, sepRegionsParameter: List[SepRegion]): Unit = {
      ...
      // println(s"CURRENT ${curr.name} :: ${curr.text}")

      // -1 if on token that doesn't start indent or whitespace,
      // otherwise number of whitespaces till beginning of line for current token
  ...
  def templateStatSeq(): (Self, List[Stat]) = {
    ...
    if (token.is[ExprIntro] && !isSoftKw(token, SoftKeyword.SkExtension)) {
      ...
      if (token.is[RightArrow]) {
        ...
        if (token.is[Indentation.Indent] && ahead(token.is[Indentation.Outdent])) {
          accept[Indentation.Indent]
          accept[Indentation.Outdent]
        }
      } else {
