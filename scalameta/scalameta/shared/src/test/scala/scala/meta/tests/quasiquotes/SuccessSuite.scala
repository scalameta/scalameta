...
class SuccessSuite extends FunSuite {
  ...
  test("1 q\"$name.this.$id\"") {
    ...
    assert(name.structure === "Name.Indeterminate(\"SuccessSuite\")")
  ...
  test("1 this variants") {
    ...
    assert(clazz.structure === "Name.Indeterminate(\"C\")")
  }

  test("2 this variants") {
    val clazz = t"C"
    assert(q"this".structure === "Term.This(Name.Anonymous())")
    assert(q"$clazz.this".structure === "Term.This(Type.Name(\"C\"))")
  }

  test("1 q\"$name.super[$name].$id\"") {
    val q"$clazz.super[$tpe].$id" = q"A.super[B].x"
    assert(clazz.structure === "Name.Indeterminate(\"A\")")
    assert(tpe.structure === "Name.Indeterminate(\"B\")")
  ...
  test("1 super variants") {
    ...
    assert(tpe1.structure === "Name.Indeterminate(\"M\")")
    assert(tpe2.structure === "Name.Indeterminate(\"M\")")
    assert(clazz1.structure === "Name.Indeterminate(\"C\")")
    assert(clazz2.structure === "Name.Indeterminate(\"C\")")
  }

  test("2 super variants") {
    val clazz = t"C"
    val tpe = t"M"
    assert(q"super".structure === "Term.Super(Name.Anonymous(), Name.Anonymous())")
    assert(q"super[$tpe]".structure === "Term.Super(Name.Anonymous(), Type.Name(\"M\"))")
    assert(q"$clazz.super".structure === "Term.Super(Type.Name(\"C\"), Name.Anonymous())")
  ...
  test("1 q\"throw $expr\"") {
    ...
    assert(expr.structure === "Term.New(Init(Type.Name(\"RuntimeException\"), Name.Anonymous(), Nil))")
  }

  test("2 q\"throw $expr\"") {
    val expr = q"new RuntimeException"
    assert(q"throw $expr".structure === "Term.Throw(Term.New(Init(Type.Name(\"RuntimeException\"), Name.Anonymous(), Nil)))")
  }
  ...
  test("1 q\"$expr: ..$@annots\"") {
    ...
    assert(annotz(0).structure === "Mod.Annot(Init(Type.Name(\"w\"), Name.Anonymous(), Nil))")
    assert(annotz(1).structure === "Mod.Annot(Init(Type.Name(\"e\"), Name.Anonymous(), Nil))")
    assert(ar.structure === "Mod.Annot(Init(Type.Name(\"r\"), Name.Anonymous(), Nil))")
  }

  test("2 q\"$expr: ..$@annots\"") {
    val mods = List(mod"@w", mod"@e")
    assert(q"foo: @q ..@$mods @r".structure === "Term.Annotate(Term.Name(\"foo\"), List(Mod.Annot(Init(Type.Name(\"q\"), Name.Anonymous(), Nil)), Mod.Annot(Init(Type.Name(\"w\"), Name.Anonymous(), Nil)), Mod.Annot(Init(Type.Name(\"e\"), Name.Anonymous(), Nil)), Mod.Annot(Init(Type.Name(\"r\"), Name.Anonymous(), Nil))))")
  }
...
//  }
  ...
  test("1 q\"new { ..$stat } with ..$inits { $param => ..$stats }\"") {
    ...
    assert(x.structure === "Init(Type.Name(\"Foo\"), Name.Anonymous(), Nil)")
  }
  ...
  test("2 q\"new { ..$stat } with ..$inits { $param => ..$stats }\"") {
    ...
    assert(a.structure === "Init(Type.Name(\"A\"), Name.Anonymous(), Nil)")
    assert(selff.structure === "Term.Param(Nil, Term.Name(\"self\"), None, None)")
  ...
  test("3 q\"new { ..$stat } with ..$inits { $param => ..$stats }\"") {
    ...
    assert(self.structure === "Term.Param(Nil, Name.Anonymous(), None, None)")
  }
  ...
  test("4 q\"new { ..$stat } with ..$inits { $param => ..$stats }\"") {
    ...
    assert(q"new {..$stats; val b = 4} with $a {$selff => ..$statz}".structure === "Term.NewAnonymous(Template(List(Defn.Val(Nil, List(Pat.Var(Term.Name(\"a\"))), None, Lit.Int(2)), Defn.Val(Nil, List(Pat.Var(Term.Name(\"b\"))), None, Lit.Int(4))), List(Init(Type.Name(\"A\"), Name.Anonymous(), Nil)), Term.Param(Nil, Term.Name(\"self\"), Some(Type.Name(\"A\")), None), Some(List(Defn.Val(Nil, List(Pat.Var(Term.Name(\"b\"))), None, Lit.Int(3))))))")
  }
  ...
  test("1 t\"$tpe ..@$annots\"") {
    ...
    assert(annots(0).structure === "Mod.Annot(Init(Type.Name(\"a\"), Name.Anonymous(), Nil))")
    assert(annots(1).structure === "Mod.Annot(Init(Type.Name(\"b\"), Name.Anonymous(), Nil))")
  }

  test("2 t\"$tpe ..@$annots\"") {
    val tpe = t"X"
    val annots = List(mod"@a", mod"@b")
    assert(t"$tpe ..@$annots".structure === "Type.Annotate(Type.Name(\"X\"), List(Mod.Annot(Init(Type.Name(\"a\"), Name.Anonymous(), Nil)), Mod.Annot(Init(Type.Name(\"b\"), Name.Anonymous(), Nil))))")
  }
  ...
  test("1 q\"..$mods val ..$pats: $tpe\"") {
    ...
    assert(mods(0).structure === "Mod.PrivateWithin(Name.Anonymous())")
  ...
  test("2 q\"..$mods val ..$pats: $tpe\"") {
    ...
    val pats = List(p"x", p"y")
    val tpe = t"T"
    assert(q"..$mods val ..$pats: $tpe".structure === "Decl.Val(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), List(Pat.Var(Term.Name(\"x\")), Pat.Var(Term.Name(\"y\"))), Type.Name(\"T\"))")
  }
  ...
  test("2 q\"..$mods var ..$pats: $tpe\"") {
    ...
    val pats = List(p"x", p"y")
    val tpe = t"T"
    assert(q"..$mods var ..$pats: $tpe".structure === "Decl.Var(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), List(Pat.Var(Term.Name(\"x\")), Pat.Var(Term.Name(\"y\"))), Type.Name(\"T\"))")
  }
  ...
  test("2 q\"..$mods val ..$pats: $tpeopt = $expr\"") {
    ...
    val tpeopt = t"T"
    val expr = q"t"
    assert(q"..$mods val ..$pats: $tpeopt = $expr".structure === "Defn.Val(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), List(Pat.Var(Term.Name(\"x\")), Pat.Var(Term.Name(\"y\"))), Some(Type.Name(\"T\")), Term.Name(\"t\"))")
  }
  ...
  test("2 q\"..$mods var ..$pats: $tpeopt = $expropt\"") {
    ...
    val tpeopt = t"T"
    val expropt = q"t"
    assert(q"..$mods var ..$pats: $tpeopt = $expropt".structure === "Defn.Var(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), List(Pat.Var(Term.Name(\"x\")), Pat.Var(Term.Name(\"y\"))), Some(Type.Name(\"T\")), Some(Term.Name(\"t\")))")
  }
  ...
  test("2 q\"..$mods type $tname[..$tparams] = $tpe\"") {
    ...
    val tparams = List(tparam"T", tparam"W")
    val tpe = t"R"
    assert(q"..$mods type $tname[..$tparams] = $tpe".structure === "Defn.Type(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), Type.Name(\"Q\"), List(Type.Param(Nil, Type.Name(\"T\"), Nil, Type.Bounds(None, None), Nil, Nil), Type.Param(Nil, Type.Name(\"W\"), Nil, Type.Bounds(None, None), Nil, Nil)), Type.Name(\"R\"))")
  }
  ...
  val q"..$mods class $tname[..$tparams] $mod (...$paramss) extends $template" = q"private final class Q[T, W] private (x: X, y: Y) extends Y"
    ...
    assert(tparams(0).structure === "Type.Param(Nil, Type.Name(\"T\"), Nil, Type.Bounds(None, None), Nil, Nil)")
    assert(tparams(1).structure === "Type.Param(Nil, Type.Name(\"W\"), Nil, Type.Bounds(None, None), Nil, Nil)")
    assert(mod.structure === "Mod.PrivateWithin(Name.Anonymous())")
    assert(paramss.toString === "List(List(x: X, y: Y))")
    assert(paramss(0)(0).structure === "Term.Param(Nil, Term.Name(\"x\"), Some(Type.Name(\"X\")), None)")
    assert(paramss(0)(1).structure === "Term.Param(Nil, Term.Name(\"y\"), Some(Type.Name(\"Y\")), None)")
    assert(template.structure === "Template(Nil, List(Init(Type.Name(\"Y\"), Name.Anonymous(), Nil)), Term.Param(Nil, Name.Anonymous(), None, None), None)")
  }
  ...
  val q"..$mods class $tname[..$tparams] $mod (...$paramss) extends $template" = q"private final class Q[T, W] protected (x: X, y: Y) extends { def m1 = 42; def m2 = 666 }"
    ...
    assert(tparams(0).structure === "Type.Param(Nil, Type.Name(\"T\"), Nil, Type.Bounds(None, None), Nil, Nil)")
    assert(tparams(1).structure === "Type.Param(Nil, Type.Name(\"W\"), Nil, Type.Bounds(None, None), Nil, Nil)")
    assert(mod.structure === "Mod.ProtectedWithin(Name.Anonymous())")
    assert(paramss.toString === "List(List(x: X, y: Y))")
    assert(paramss(0)(0).structure === "Term.Param(Nil, Term.Name(\"x\"), Some(Type.Name(\"X\")), None)")
    assert(paramss(0)(1).structure === "Term.Param(Nil, Term.Name(\"y\"), Some(Type.Name(\"Y\")), None)")
    assert(template.structure === "Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(Nil, Term.Name(\"m1\"), Nil, Nil, None, Lit.Int(42)), Defn.Def(Nil, Term.Name(\"m2\"), Nil, Nil, None, Lit.Int(666)))))")
  }
  ...
  test("3 q\"..$mods object $name extends $template\"") {
    ...
    val name = q"Q"
    val template = template"F { def m = 42 }"
    assert(q"..$mods object $name extends $template".structure === "Defn.Object(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), Term.Name(\"Q\"), Template(Nil, List(Init(Type.Name(\"F\"), Name.Anonymous(), Nil)), Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(Nil, Term.Name(\"m\"), Nil, Nil, None, Lit.Int(42))))))")
  }
  ...
  test("3 q\"package object $name extends $template\"") {
    val name = q"Q"
    val template = template"F { def m = 42 }"
    assert(q"package object $name extends $template".structure === "Pkg.Object(Nil, Term.Name(\"Q\"), Template(Nil, List(Init(Type.Name(\"F\"), Name.Anonymous(), Nil)), Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(Nil, Term.Name(\"m\"), Nil, Nil, None, Lit.Int(42))))))")
  }
  ...
  test("1 q\"package $ref { ..$stats }\"") {
    ...
    assert(stats(0).structure === "Defn.Class(Nil, Type.Name(\"A\"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None))")
    assert(stats(1).structure === "Defn.Object(Nil, Term.Name(\"B\"), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None))")
  }

  test("2 q\"package $ref { ..$stats }\"") {
    val ref = q"p"
    val stats = List(q"class A", q"object B")
    assert(q"package $ref { ..$stats }".structure === "Pkg(Term.Name(\"p\"), List(Defn.Class(Nil, Type.Name(\"A\"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None)), Defn.Object(Nil, Term.Name(\"B\"), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None))))")
  }
  ...
  test("3 q\"package $ref { ..$stats }\"") {
    val ref = q"p.a"
    val stats = List(q"class A", q"object B")
    assert(q"package $ref { ..$stats }".structure === """Pkg(Term.Select(Term.Name("p"), Term.Name("a")), List(Defn.Class(Nil, Type.Name("A"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None)), Defn.Object(Nil, Term.Name("B"), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), None))))""")
  }
  ...
  test("1 q\"..$mods def this(...$paramss)\"") {
   ...
   test("2 q\"..$mods def this(...$paramss)\"") {
     ...
     val paramss = List(List(param"x: X", param"x: Y"))
     assert(q"..$mods def this(...$paramss)".structure === "Ctor.Primary(List(Mod.PrivateWithin(Name.Anonymous())), Name.Anonymous(), List(List(Term.Param(Nil, Term.Name(\"x\"), Some(Type.Name(\"X\")), None), Term.Param(Nil, Term.Name(\"x\"), Some(Type.Name(\"Y\")), None))))")
   }
  ...
  test("1 q\"..$mods def this(...$paramss) = $expr\"") {
    ...
    assert(paramss(0)(0).structure === "Term.Param(Nil, Term.Name(\"x\"), Some(Type.Name(\"X\")), None)")
    assert(paramss(0)(1).structure === "Term.Param(Nil, Term.Name(\"y\"), Some(Type.Name(\"Y\")), None)")
    assert(init.structure === "Init(Type.Singleton(Term.This(Name.Anonymous())), Name.Anonymous(), List(List(Term.Name(\"foo\"), Term.Name(\"bar\"))))")
  }
  ...
  test("2 param\"..$mods $paramname: $tpeopt = $expropt\"") {
    ...
    val tpeopt = t"X"
    val expropt = q"42"
    assert(param"..$mods $paramname: $tpeopt = $expropt".structure === "Term.Param(List(Mod.PrivateWithin(Name.Anonymous()), Mod.Final()), Term.Name(\"x\"), Some(Type.Name(\"X\")), Some(Lit.Int(42)))")
  }
  ...
  test("2 init\"$tpe(...$exprss)\"") {
    ...
    assert(init"$tpe(...$exprss)".structure === "Init(Type.Name(\"C\"), Name.Anonymous(), List(List(Lit.Int(40)), List(Lit.Int(2))))")
  }
  ...
  test("2 init\"this(...$exprss)\"") {
    val exprss = List(List(q"40"), List(q"2"))
    assert(init"this(...$exprss)".structure === "Init(Type.Singleton(Term.This(Name.Anonymous())), Name.Anonymous(), List(List(Lit.Int(40)), List(Lit.Int(2))))")
  }
  ...
  test("1 template\"{ ..$stats } with ..$inits { $param => ..$stats }\"") {
    ...
    assert(inits(0).structure === "Init(Type.Name(\"T\"), Name.Anonymous(), Nil)")
    assert(inits(1).structure === "Init(Type.Name(\"U\"), Name.Anonymous(), Nil)")
    assert(param.structure === "Term.Param(Nil, Term.Name(\"self\"), Some(Type.Name(\"Z\")), None)")
   ...
   test("2 template\"{ ..$stats } with ..$inits { $param => ..$stats }\"") {
     ...
     assert(template"{ ..$stats1 } with ..$inits { $param => ..$stats2 }".structure === "Template(List(Defn.Val(Nil, List(Pat.Var(Term.Name(\"a\"))), None, Lit.Int(2)), Defn.Val(Nil, List(Pat.Var(Term.Name(\"b\"))), None, Lit.Int(2))), List(Init(Type.Name(\"T\"), Name.Anonymous(), Nil), Init(Type.Name(\"U\"), Name.Anonymous(), Nil)), Term.Param(Nil, Term.Name(\"self\"), Some(Type.Name(\"S\")), None), Some(List(Defn.Def(Nil, Term.Name(\"m\"), Nil, Nil, None, Lit.Int(2)), Defn.Def(Nil, Term.Name(\"n\"), Nil, Nil, None, Lit.Int(2)))))")
   }

  test("1 mod\"@$expr\"") {
    val mod"@$expr" = mod"@a"
    assert(expr.structure === "Mod.Annot(Init(Type.Name(\"a\"), Name.Anonymous(), Nil))")
  }

  test("2 mod\"@$expr\"") {
    val expr = mod"@a"
    assert(mod"@$expr".structure === "Mod.Annot(Init(Type.Name(\"a\"), Name.Anonymous(), Nil))")
  }

  test("1 mod\"private[$name]\"") {
    val mod"private[$name]" = mod"private[X]"
    assert(name.structure === "Name.Indeterminate(\"X\")")
  }

  test("2 mod\"private[$name]\"") {
    val mod"private[$name]" = mod"private"
    assert(name.structure === "Name.Anonymous()")
  }
  ...
  test("4 mod\"private[$name]\"") {
    val name = q"q"
    assert(mod"private[$name]".structure === "Mod.PrivateWithin(Term.Name(\"q\"))")
  }
  ...
  test("4 mod\"protected[$name]\"") {
    val name = q"q"
    assert(mod"protected[$name]".structure === "Mod.ProtectedWithin(Term.Name(\"q\"))")
  }
  ...
  test("1 importee\"$iname\"") {
    ...
    assert(iname.structure === "Name.Indeterminate(\"x\")")
  }

  test("2 importee\"$iname\"") {
    // $iname can't be constructed, only extracted from importee"..." and mod"..."
    val importee"$iname" = importee"x"
    assert(importee"$iname".structure === "Importee.Name(Name.Indeterminate(\"x\"))")
  }

  test("1 importee\"$iname => $iname\"") {
    val importee"$iname1 => $iname2" = importee"x => y"
    assert(iname1.structure === "Name.Indeterminate(\"x\")")
    assert(iname2.structure === "Name.Indeterminate(\"y\")")
  }

  test("2 importee\"$iname => $iname\"") {
    // $iname can't be constructed, only extracted from importee"..." and mod"..."
    val importee"$iname1 => $iname2" = importee"x => y"
    assert(importee"$iname1 => $iname2".structure === "Importee.Rename(Name.Indeterminate(\"x\"), Name.Indeterminate(\"y\"))")
  }
  ...
  // $iname can't be constructed, only extracted from importee"..." and mod"..."
  val importee"$iname => _" = importee"x => _"
    assert(importee"$iname => _".structure === "Importee.Unimport(Name.Indeterminate(\"x\"))")
  }
  ...
  test("1 source\"..$stats\"") {
    ...
    assert(stats(0).structure === "Defn.Class(Nil, Type.Name(\"A\"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Val(Nil, List(Pat.Var(Term.Name(\"a\"))), None, Lit.Char('a'))))))")
  }
  ...
  test("unquote T into Option[List[T]]") {
    val stat = q"def x = 42"
    assert(q"class C { $stat }".structure === "Defn.Class(Nil, Type.Name(\"C\"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(Nil, Term.Name(\"x\"), Nil, Nil, None, Lit.Int(42))))))")
  }

  test("unquote List[T] into Option[List[T]]") {
    val stats = List(q"def x = 42")
    assert(q"class C { ..$stats }".structure === "Defn.Class(Nil, Type.Name(\"C\"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(Nil, Term.Name(\"x\"), Nil, Nil, None, Lit.Int(42))))))")
  }

  test("extract T from Option[List[T]]") {
    val q"class $_ { $stat }" = q"class C { def x = 42 }"
    assert(stat.structure === "Defn.Def(Nil, Term.Name(\"x\"), Nil, Nil, None, Lit.Int(42))")
  }

  test("extract List[T] from Option[List[T]]") {
    val q"class $_ { ..$stats }" = q"class C { def x = 42 }"
    assert(stats.structure === "List(Defn.Def(Nil, Term.Name(\"x\"), Nil, Nil, None, Lit.Int(42)))")
  }

  test("extract Nil from Option[List[T]]") {
    val q"class $_ { ..$stats }" = q"class C"
    assert(stats.structure === "List()")
  }
  ...
  test("ellipses in template stats") {
    ...
    assert(tree.structure === """Defn.Class(Nil, Type.Name("C"), Nil, Ctor.Primary(Nil, Name.Anonymous(), Nil), Template(Nil, Nil, Term.Param(Nil, Name.Anonymous(), None, None), Some(List(Defn.Def(List(Mod.PrivateWithin(Name.Anonymous())), Term.Name("x"), Nil, Nil, None, Lit.Int(2))))))""")
  }
