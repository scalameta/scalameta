...
object Versions {
  ...
  val EarliestScalaVersions = EarliestScala2Versions :+ Scala3Versions.head
  ...
  private def getVersions3(patch: String*) = {
    if (patch.length > 4) throw new Exception(s"Too many versions for scala3: ${patch.length} > 4")
    patch.map("3." + _)
  }
