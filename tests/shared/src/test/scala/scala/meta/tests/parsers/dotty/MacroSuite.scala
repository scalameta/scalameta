...
class MacroSuite extends BaseDottySuite {
  ...
  test("macro-quote-expr") {
    ...
    runTestAssert[Stat]("if (b) '{ true } else '{ false }")(
      Term.If(tname("b"), Term.QuotedMacroExpr(List(Lit.Boolean(true))), Term.QuotedMacroExpr(List(Lit.Boolean(false))))
    )
    runTestAssert[Stat]("'{ val y = a; ${ env } }")(
      Term.QuotedMacroExpr(List(
    ...
    runTestAssert[Stat]("x match { case '{ a } => 1 }", assertLayout = false)(
      Term.Match(tname("x"), List(Case(Pat.Macro(Term.QuotedMacroExpr(List(tname("a")))), None, Lit.Int(1))))
    )
  }
...
Term.QuotedMacroType(Type.Apply(pname("List"), List(Type.Macro(Term.SplicedMacroExpr(List(Term.ApplyType(tname("summon"), List(Type.Apply(pname("Type"), List(pname("T")))))))))))
    )
  ...
  test("macro-splice") {
    runTestAssert[Stat]("${ powerCode('x) }")(
      Term.SplicedMacroExpr(List(Term.Apply(tname("powerCode"),
       List(Lit.Symbol('x)))))
    )
    runTestAssert[Stat]("${ x = 'y; 1 }")(
      Term.SplicedMacroExpr(List(Term.Assign(tname("x"), Lit.Symbol('y)), Lit.Int(1)))
    )
    runTestAssert[Stat]("${ assertImpl('{ x != $y }) }")(
      Term.SplicedMacroExpr(List(
        Term.Apply(tname("assertImpl"), List(Term.QuotedMacroExpr(List(Term.ApplyInfix(tname("x"), tname("!="), Nil, List(tname("$y")))))))
        ))
    )
  }
  ...
  test("macro-quote-complex") {
    runTestAssert[Stat]("'{ ClassTag[T](${ Expr(ct.runtimeClass.asInstanceOf[Class[T]]) }) }")(
      Term.QuotedMacroExpr(List(Term.Apply(Term.ApplyType(tname("ClassTag"), List(pname("T"))),
        List(Term.SplicedMacroExpr(List(Term.Apply(tname("Expr"), List(Term.ApplyType(Term.Select(
          Term.Select(tname("ct"), tname("runtimeClass")), tname("asInstanceOf")), List(Type.Apply(pname("Class"), List(pname("T")))))))))))))
    )

    // TODO: it generates () around postfix argument, check if it is necessary 
    runTestAssert[Stat]("'{ ${ summon[H].toExpr(tup.head) } *: ${ summon[T].toExpr(tup.tail) } }", assertLayout = false)(
      Term.QuotedMacroExpr(List(Term.ApplyInfix(
        Term.SplicedMacroExpr(List(Term.Apply(Term.Select(Term.ApplyType(tname("summon"), List(pname("H"))), tname("toExpr")), List(Term.Select(tname("tup"), tname("head")))))),
        tname("*:"),
        Nil,
        List(Term.SplicedMacroExpr(List(Term.Apply(Term.Select(Term.ApplyType(tname("summon"), List(pname("T"))), tname("toExpr")), List(Term.Select(tname("tup"), tname("tail"))))))))
        ))
    )
  }
}
