...
class DialectSuite extends FunSuite {
  ...
  test("scala3 toString")(assertEquals(Scala3.toString, "Scala39"))

  test("scala3copy toString")(assertEquals(scala3copy.toString, "Scala39"))

  test("scala3 without indentation toString")(
    assertEquals(Scala3.withAllowSignificantIndentation(false).toString, "Dialect()"),
  )

  test("scala3.unquoteTerm toString")(assertEquals(Scala3.unquoteTerm(true).toString, "Scala39"))
