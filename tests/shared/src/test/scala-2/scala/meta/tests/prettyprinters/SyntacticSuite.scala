...
class SyntacticSuite extends scala.meta.tests.parsers.ParseSuite {
  ...
  test("parentheses in patterns")(assertEquals(
  ...
  test("#3372") {
    ...
    val layout =
      """|// c1
         ...
         |val foo = bar /* c3 */ // c4
         |""".stripMargin
    val tree = Defn.Val.newBuilder(Nil, List(patvar("foo")), None, tname("bar"))
      .begComment(Seq("// c1", "/* c2 */")).endComment(Seq("/* c3 */", "// c4")).result()
