...
abstract class BaseTokenizerCoverageSuite extends FunSuite {
  ...
  def tmod[T]: Projection[Defn.Class, T] =
    projection[Defn.Class, T](_.tparams.head.mods.head.asInstanceOf[T])
  def ctorMod[T]: Projection[Defn.Class, T] =
    projection[Defn.Class, T](_.ctor.paramss.head.head.mods.head.asInstanceOf[T])
  ...
  implicit val defParams: Projection[Decl.Def, Term.Param] =
    projection[Decl.Def, Term.Param](_.paramss.head.head)
  implicit val importImporter: Projection[Import, Importer] =
    projection[Import, Importer](_.importers.head)
  ...
  implicit val anonTemplate: Projection[Term.NewAnonymous, Template] =
    projection[Term.NewAnonymous, Template](_.templ)
  implicit val classTemplate: Projection[Defn.Class, Template] =
    projection[Defn.Class, Template](_.templ)
  implicit val classCtorSecondary: Projection[Defn.Class, Ctor.Secondary] =
    projection[Defn.Class, Ctor.Secondary](_.templ.stats.head.asInstanceOf[Ctor.Secondary])
  implicit val defTypeParam: Projection[Decl.Def, Type.Param] =
    projection[Decl.Def, Type.Param](_.tparams.head)
  ...
  private def testName(source: String, tree: Tree): fansi.Str = {
    val fullClassName =
      tree.getClass.toString.stripPrefix("class scala.meta.").replaceAllLiterally("$", ".")
