...
object NamedApplyBlockMethods/*<=example.NamedApplyBlockMethods.*/ {
  ...
  def recursive/*<=example.NamedApplyBlockMethods.recursive().*/ = foo/*=>example.NamedApplyBlockMethods.foo().*/(local/*=>example.NamedApplyBlockMethods.local.*/, c/*=>example.NamedApplyBlockMethods.foo().(c)*/ = foo/*=>example.NamedApplyBlockMethods.foo().*/(local/*=>example.NamedApplyBlockMethods.local.*/, c = 3))
}
