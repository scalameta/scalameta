...
class ReflectionSuite extends AstSuite {
  ...
  test("If") {
    ...
    assert(a4.toString === "field Term.If.status: scala.meta.internal.semantic.Status (auxiliary)")
  ...
  test("Term.Name") {
    ...
    assert(a3.toString === "field Term.Name.sigma: scala.meta.internal.semantic.Sigma (auxiliary)")
    assert(a4.toString === "field Term.Name.status: scala.meta.internal.semantic.Status (auxiliary)")
    assert(a5.toString === "field Term.Name.expansion: scala.meta.internal.semantic.Expansion (auxiliary)")
  }
