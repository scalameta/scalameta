...
// TODO: this test should be removed once show[Code] calls InferTokens.
class CompareParentsSuite extends FunSuite {
  ...
  private def printCodes(codes: String*)(errOpt: Option[Throwable]): Unit = {
    println("====================================================")
    codes foreach {c => 
      println(c)
      println("----------------------------------------------------")
  ...
  def loop(dir: File): Unit = {
    def linePositionTest(src: File): Unit = {
      test("Testing number of parentheses for file " + src.getAbsolutePath) {
        ...
        /* Trying parsing show[Code] output */
        val showCodeOutput = parsed.show[Code]
        Try(showCodeOutput.parse[Source]) match {
            case Failure(_) => /* Could not parse show[Code] output. Cannot compare the outputs */
