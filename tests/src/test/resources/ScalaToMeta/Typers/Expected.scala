...
trait Typers extends Adaptations with Tags with TypersTracking with PatternTypers { self: Analyzer =>
  ...
  class SilentTypeError private (val errors: List[AbsTypeError]) extends SilentResult[Nothing] {
    ...
    def reportableErrors = errors match {
      case (e1: AmbiguousImplicitTypeError) +: _ => List(e1)
      case all => all
    }
  }
  ...
  abstract class Typer(context0: Context) extends TyperDiagnostics with Adaptation with Tag with PatternTyper with TyperContextErrors {
    ...
    def checkNonCyclic(pos: Position, tp: Type): Boolean = {
      ...
      tp match {
        case TypeRef(pre, sym, args) => checkNotLocked(sym) && (!sym.isNonClassType || checkNonCyclic(pos, appliedType(pre.memberInfo(sym), args), sym))
        case SingleType(pre, sym) => checkNotLocked(sym)
        case st: SubType => checkNonCyclic(pos, st.supertype)
        case ct: CompoundType => ct.parents.forall(x => checkNonCyclic(pos, x))
        case _ => true
      }
    }
    ...
    @inline final def typerWithLocalContext[T](c: Context)(f: Typer => T): T = c.reporter.propagatingErrorsTo(context.reporter)(f(newTyper(c)))
    ...
    def stabilizeFun(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      tree.tpe match {
        case MethodType(_, _) if stabilizable => tree.setType(MethodType(Nil, singleType(pre, sym)))
        case _ => tree
      }
    }
    ...
    def silent[T](op: Typer => T, reportAmbiguousErrors: Boolean = context.ambiguousErrors, newtree: Tree = context.tree): SilentResult[T] = {
      ...
      @inline def wrapResult(reporter: ContextReporter, result: T) = if (reporter.hasErrors) {
        stopStats()
      ...
      try if (context.reportErrors || reportAmbiguousErrors != context.ambiguousErrors || newtree != context.tree) {
        ...
        if (!context1.reporter.hasErrors) context1.reporter.emitWarnings()
        wrapResult(context1.reporter, result)
      } else {
        assert(context.bufferErrors || isPastTyper, "silent mode is not available past typer")
        context.reporter.withFreshErrorBuffer(wrapResult(context.reporter, op(this)))
      } catch {
    ...
    def checkExistentialsFeature(pos: Position, tpe: Type, prefix: String) = tpe match {
      case extp: ExistentialType if !extp.isRepresentableWithWildcards => checkFeature(pos, ExistentialsFeature, prefix + " " + tpe)
      case _ =>
    }
    def samToFunctionType(tp: Type, sam: Symbol = NoSymbol): Type = {
      ...
      def correspondingFunctionSymbol = {
        ...
        if (numVparams > definitions.MaxFunctionArity) NoSymbol else FunctionClass(numVparams)
      }
      if (samSym.exists && samSym.owner != correspondingFunctionSymbol) wildcardExtrapolation(normalize(tp.memberInfo(samSym))) else NoType
    }
    protected def adapt(tree: Tree, mode: Mode, pt: Type, original: Tree = EmptyTree): Tree = {
      ...
      def adaptToImplicitMethod(mt: MethodType): Tree = {
        ...
        if (context.reporter.hasErrors) setError(tree) else withCondConstrTyper(treeInfo.isSelfOrSuperConstrCall(tree))(typer1 => if (original != EmptyTree && pt != WildcardType) typer1.silent(tpr => {
          val withImplicitArgs = tpr.applyImplicitArgs(tree)
          if (tpr.context.reporter.hasErrors) tree else tpr.typed(withImplicitArgs, mode, pt)
        }).orElse(_ => {
      ...
      def instantiateToMethodType(mt: MethodType): Tree = {
        ...
        if (!meth.isConstructor && (isFunctionType(pt) || samOf(pt).exists)) {
      ...
      def adaptType(): Tree = {
        def properTypeRequired = tree.hasSymbolField && !context.inTypeConstructorAllowed && !context.unit.isJava
    ...
    def adaptAfterOverloadResolution(tree: Tree, mode: Mode, pt: Type = WildcardType, original: Tree = EmptyTree): Tree = adapt(tree, mode, pt, original)
    ...
    def adaptToMember(qual: Tree, searchTemplate: Type, reportAmbiguous: Boolean = true, saveErrors: Boolean = true): Tree = if (isAdaptableWithView(qual)) {
      qual.tpe.dealiasWiden match {
        case et: ExistentialType => qual.setType(et.skolemizeExistential(context.owner, qual))
        case _ =>
      }
    ...
    def adaptToMemberWithArgs(tree: Tree, qual: Tree, name: Name, mode: Mode, reportAmbiguous: Boolean, saveErrors: Boolean): Tree = {
      def onError(reportError: => Tree): Tree = context.tree match {
        case Apply(tree1, args) if (tree1 eq tree) && args.nonEmpty => silent(_.typedArgs(args.map(_.duplicate), mode)).filter(xs => !xs.exists(_.isErrorTyped)).map(xs => adaptToArguments(qual, name, xs, WildcardType, reportAmbiguous, saveErrors)).orElse(_ => reportError)
        case _ => reportError
      }
...
This restriction is planned to be removed in subsequent releases.""")
      object checkEphemeralDeep extends Traverser {
        override def traverse(tree: Tree): Unit = if (isValueClass) {
          tree match {
            case _: ModuleDef => implRestriction(tree, "nested object")
            case cd: ClassDef if !cd.symbol.isAnonymousClass => implRestriction(tree, "nested class")
            case Select(sup @ Super(qual, mix), selector) if selector != nme.CONSTRUCTOR && qual.symbol == clazz && mix != tpnme.EMPTY => implRestriction(sup, "qualified super reference")
            case _ =>
          }
          super.traverse(tree)
        }
    ...
    private def validateDerivedValueClass(clazz: Symbol, body: List[Tree]) = {
      ...
      for (tparam <- clazz.typeParams) if (tparam.hasAnnotation(definitions.SpecializedClass)) context.error(tparam.pos, "type parameter of value class may not be specialized")
    }
    ...
    def validateParentClasses(parents: List[Tree], selfType: Type): scala.Unit = {
      ...
      def validateParentClass(parent: Tree, superclazz: Symbol) = if (!parent.isErrorTyped) {
        ...
        if (!isPastTyper && psym.hasDeprecatedInheritanceAnnotation && !sameSourceFile && !context.owner.ownerChain.exists(x => x.isDeprecated || x.hasBridgeAnnotation)) {
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      if (clazz.isNonBottomSubClass(ClassfileAnnotationClass) && clazz != ClassfileAnnotationClass) if (!clazz.owner.isPackageClass) context.error(clazz.pos, "inner classes cannot be classfile annotations") else if (clazz != SerialVersionUIDAttr) restrictionWarning(cdef.pos, unit, """|subclassing Classfile does not
             |make your annotation visible at runtime.  If that is what
             |you want, you must write the annotation class in Java.""".stripMargin)
      warnTypeParameterShadow(tparams1, clazz)
      if (!isPastTyper) for (ann <- clazz.getAnnotation(DeprecatedAttr)) {
    ...
    def typedTemplate(templ0: Template, parents1: List[Tree]): Template = {
      ...
      if (clazz.isTrait) for (decl <- clazz.info.decls; if decl.isTerm && decl.isEarlyInitialized) context.warning(decl.pos, "Implementation restriction: early definitions in traits are not initialized before the super class is initialized.")
    ...
    private def typedValDefImpl(vdef: ValDef) = {
      ...
      val rhs1 = if (vdef.rhs.isEmpty) {
        ...
        val tpt2 = if (sym.hasDefault) {
          ...
          if (sym.hasFlag(BYNAMEPARAM)) if (tpt1.tpe.typeArgs.isEmpty) WildcardType else subst(tpt1.tpe.typeArgs(0)) else subst(tpt1.tpe)
        } else tpt1.tpe
    ...
    def computeParamAliases(clazz: Symbol, vparamss: List[List[ValDef]], rhs: Tree): scala.Unit = {
      ...
      def decompose(call: Tree): (Tree, List[Tree]) = call match {
        ...
        case Apply(fn, args) =>
          foreachSubTreeBoundTo(args, clazz)(tree => {
            ...
            tree match {
              case This(qual) => pending += SuperConstrArgsThisReferenceError(tree)
              case _ => ()
            }
          })
      ...
      if (superConstr.symbol.isPrimaryConstructor && !superClazz.isJavaDefined && sameLength(superParamAccessors, superArgs)) for ((superAcc, superArg @ Ident(name)) <- superParamAccessors.zip(superArgs)) if (mexists(vparamss)(_.symbol == superArg.symbol)) {
      ...
      ddef.rhs match {
        case Block(stats, expr) =>
          ...
          foreachSubTreeBoundTo(List(selfConstructorCall), clazz)({
            case tree @ This(qual) => pending += SelfConstrArgsThisReferenceError(tree)
            case _ => ()
          })
        case _ =>
      }
    ...
    private def foreachSubTreeBoundTo[A](trees: List[Tree], clazz: Symbol)(f: Tree => Unit): Unit = for (tree <- trees; subTree <- tree) {
    ...
    def typedDefDef(ddef: DefDef): DefDef = {
      ...
      if (!isPastTyper && meth.isPrimaryConstructor) for (vparams <- ddef.vparamss; vd <- vparams) if (vd.mods.isParamAccessor) namer.validateParam(vd)
      ...
      if (!isPastTyper) {
        ...
        for (p <- allParams; n <- p.deprecatedParamName) if (allParams.exists(p1 => p1.name == n || p != p1 && p1.deprecatedParamName.exists(_ == n))) DeprecatedParamNameError(p, n)
        ...
        if (meth.isImplicit && !meth.isSynthetic) meth.info.paramss match {
          case List(param) :: _ if !param.isImplicit => checkFeature(ddef.pos, ImplicitConversionsFeature, meth.toString)
          case _ =>
        }
      }
    ...
    private def enterLabelDef(stat: Tree): scala.Unit = stat match {
      case ldef @ LabelDef(_, _, _) => if (ldef.symbol == NoSymbol) ldef.symbol = namer.enterInScope(context.owner.newLabel(ldef.name, ldef.pos).setInfo(MethodType(List(), UnitTpe)))
      case _ =>
    }
    ...
    def typedBlock(block0: Block, mode: Mode, pt: Type): Block = {
      ...
      try {
        ...
        if (phaseId(currentPeriod) <= currentRun.typerPhase.id) block match {
          case Block(List(classDef @ ClassDef(_, _, _, _)), Apply(Select(New(_), _), _)) =>
            ...
            val toHide = classDecls.filter(member => member.isTerm && member.isPossibleInRefinement && member.isPublic && !matchesVisibleMember(member)).map(member => member.resetFlag(PROTECTED | LOCAL).setFlag(PRIVATE | SYNTHETIC_PRIVATE).setPrivateWithin(NoSymbol))
            syntheticPrivates ++= toHide
          case _ =>
        }
        val stats1 = if (isPastTyper) block.stats else block.stats.flatMap(stat => stat match {
          case vd @ ValDef(_, _, _, _) if vd.symbol.isLazy => namer.addDerivedTrees(Typer.this, vd)
          case _ => stat :: Nil
        })
      ...
      } finally syntheticPrivates.foreach(_.resetFlag(SYNTHETIC_PRIVATE))
    }
    def typedCase(cdef: CaseDef, pattpe: Type, pt: Type): CaseDef = {
      for (Apply(_, xs) <- cdef.pat; x <- xs.dropRight(1); if treeInfo.isStar(x)) StarPositionInPatternError(x)
    ...
    def synthesizePartialFunction(paramName: TermName, paramPos: Position, paramSynthetic: Boolean, tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def mkParam(methodSym: Symbol, tp: Type = argTp) = methodSym.newValueParameter(paramName, paramPos.focus, SYNTHETIC).setInfo(tp)
    ...
    def synthesizeSAMFunction(sam: Symbol, fun: Function, resPt: Type, samClassTp: Type, mode: Mode): Tree = {
      ...
      val samDef = DefDef(Modifiers(FINAL | OVERRIDE | SYNTHETIC), sam.name.toTermName, Nil, List(fun.vparams), TypeTree(samMethTp.finalResultType).setPos(sampos.focus), Apply(Ident(bodyName), fun.vparams.map(gen.paramToArg)))
      val serializableParentAddendum = if (typeIsSubTypeOfSerializable(samClassTp)) Nil else List(TypeTree(SerializableTpe))
      val classDef = ClassDef(Modifiers(FINAL), tpnme.ANON_FUN_NAME, tparams = Nil, gen.mkTemplate(parents = TypeTree(samClassTpFullyDefined) :: serializableParentAddendum, self = emptyValDef, constrMods = NoMods, vparamss = ListOfNil, body = List(samDef), superPos = sampos.focus))
      ...
      if (block.exists(_.isErroneous)) context.error(fun.pos, s"Could not derive subclass of $samClassTp\n (with SAM `def $sam$samMethTp`)\n based on: $fun.")
    ...
    private def typedFunction(fun: Function, mode: Mode, pt: Type): Tree = {
      ...
      val sam = if (pt.typeSymbol == FunctionSymbol) NoSymbol else samOf(pt)
      ...
      def typedStat(stat: Tree): Tree = if (context.owner.isRefinementClass && !treeInfo.isDeclarationOrTypeDef(stat)) OnlyDeclarationsError(stat) else stat match {
        ...
        case _ =>
          if (localTarget && !includesTargetPos(stat)) stat else {
            ...
            if (!isPastTyper && treeInfo.isPureExprForWarningPurposes(result)) context.warning(stat.pos, "a pure expression does nothing in statement position; " + "you may be omitting necessary parentheses")
            result
          }
      }
  ...
  """ + (e1.sym.getClass, e1.sym.info, e1.sym.ownerChain))
      ...
      def addSynthetics(stats: List[Tree]): List[Tree] = {
        ...
        while (moreToAdd) {
          ...
          for (sym <- scope; tree <- context.unit.synthetics.get(sym); if shouldAdd(sym)) {
        ...
        if (newStats.isEmpty) stats else {
          def matches(stat: Tree, synt: Tree) = (stat, synt) match {
            case (DefDef(_, statName, _, _, _, _), DefDef(mods, syntName, _, _, _, _)) => mods.hasDefault && syntName.toString.startsWith(statName.toString)
            case (ClassDef(_, className, _, _), ModuleDef(_, moduleName, _)) => className.toTermName == moduleName
            case (ClassDef(cmods, cname, _, _), DefDef(dmods, dname, _, _, _, _)) => cmods.isImplicit && dmods.isImplicit && cname.toTermName == dname
            case _ => false
          }
      ...
      exists2(formals, args)({
        case (formal, Function(vparams, _)) =>
          vparams.exists(_.tpt.isEmpty) && vparams.length <= MaxFunctionArity && (formal.baseType(FunctionClass(vparams.length)) match {
            case TypeRef(_, _, formalargs) => exists2(formalargs, vparams)((formal, vparam) => vparam.tpt.isEmpty && tparams.exists(formal.contains)) && tparams.forall(isLowerBounded)
            case _ => false
          })
        case _ =>
          false
    ...
    def doTypedApply(tree: Tree, fun0: Tree, args: List[Tree], mode: Mode, pt: Type): Tree = {
      ...
      def preSelectOverloaded(fun: Tree): Tree = if (fun.hasSymbolField && fun.symbol.isOverloaded) {
        def shapeType(arg: Tree): Type = arg match {
          ...
          case AssignOrNamedArg(Ident(name), rhs) => NamedType(name, shapeType(rhs))
          case _ => NothingTpe
        }
        ...
        if (sym == NoSymbol) fun else adaptAfterOverloadResolution(fun.setSymbol(sym).setType(pre.memberType(sym)), mode.forFunMode)
      } else fun
      ...
      fun.tpe match {
        case OverloadedType(pre, alts) =>
          def handleOverloaded = {
            ...
            if (context.reporter.hasErrors) setError(tree) else {
        ...
        case mt @ MethodType(params, _) =>
          ...
          if (!sameLength(formals, args) || args.exists(isNamedArg) || isNamedApplyBlock(fun)) if (dyna.isApplyDynamicNamed(fun) && isDynamicRewrite(fun)) dyna.typedNamedApply(tree, fun, args, mode, pt) else tryNamesDefaults else {
...
""" + "currently not supported; ignoring arguments " + args)
    ...
    def packCaptured(tpe: Type): Type = {
      ...
      for (tp <- tpe) if (isCapturedExistential(tp.typeSymbol)) captured += tp.typeSymbol
          ...
          case TypeRef(pre, sym, args) =>
            if (sym.isAliasType && containsLocal(tp) && (tp.dealias ne tp)) apply(tp.dealias) else {
              if (pre.isVolatile) pre match {
                case SingleType(_, sym) if sym.isSynthetic && isPastTyper => debuglog(s"ignoring volatility of prefix in pattern matcher generated inferred type: $tp")
                case _ => InferTypeWithVolatileTypeSelectionError(tree, pre)
              }
              mapOver(tp)
            }
          case _ =>
        ...
        for (t <- tp) {
          t match {
            ...
            case AnnotatedType(annots, _) =>
              for (annot <- annots; arg <- annot.args) arg match {
                case Ident(_) => if (!arg.tpe.typeSymbol.hasFlag(EXISTENTIAL)) addIfLocal(arg.symbol, arg.tpe)
                case _ => ()
              }
            case _ =>
          }
    ...
    protected def typedExistentialTypeTree(tree: ExistentialTypeTree, mode: Mode): Tree = {
      for (wc <- tree.whereClauses) if (wc.symbol == NoSymbol) {
        namer.enterSym(wc)
        wc.symbol.setFlag(EXISTENTIAL)
      } else context.scope.enter(wc.symbol)
      val whereClauses1 = typedStats(tree.whereClauses, context.owner)
      for (vd @ ValDef(_, _, _, _) <- whereClauses1) if (vd.symbol.tpe.isVolatile) AbstractionFromVolatileTypeError(vd)
    ...
    object dyna {
      ...
      def typedNamedApply(orig: Tree, fun: Tree, args: List[Tree], mode: Mode, pt: Type): Tree = {
        def argToBinding(arg: Tree): Tree = arg match {
          case AssignOrNamedArg(i @ Ident(name), rhs) => atPos(i.pos.withEnd(rhs.pos.end))(gen.mkTuple(List(atPos(i.pos)(CODE.LIT(name.toString)), rhs)))
      ...
      def mkInvoke(context: Context, tree: Tree, qual: Tree, name: Name): Option[Tree] = {
        ...
        acceptsApplyDynamicWithType(qual, name).map(tp => {
          ...
          def findSelection(t: Tree): Option[(TermName, Tree)] = t match {
            ...
            case _ =>
              t.children.flatMap(findSelection).headOption
          }
    ...
    def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def typedVirtualizedMatch(tree: Match): Tree = {
        ...
        if (selector == EmptyTree) if (pt.typeSymbol == PartialFunctionClass) synthesizePartialFunction(newTermName(context.unit.fresh.newName("x")), tree.pos, paramSynthetic = true, tree, mode, pt) else {
          ...
          val params = for (i <- List.range(0, arity)) yield atPos(tree.pos.focusStart)(ValDef(Modifiers(PARAM | SYNTHETIC), unit.freshTermName("x" + i + "$"), TypeTree(), EmptyTree))
      ...
      def typedApply(tree: Apply) = tree match {
        ...
        case Apply(fun, args) =>
          normalTypedApply(tree, fun, args) match {
            ...
            case Apply(Select(fun, nme.apply), _) if treeInfo.isSuperConstrCall(fun) => TooManyArgumentListsForConstructor(tree)
            case tree1 => tree1
          }
      }
...
""")}
      ...
      def typedCompoundTypeTree(tree: CompoundTypeTree) = {
        ...
        for (stat <- templ.body; if !treeInfo.isDeclarationOrTypeDef(stat)) OnlyDeclarationsError(stat)
      ...
      def issueTryWarnings(tree: Try): Try = {
        ...
        if (!isPastTyper) tree match {
          case Try(_, Nil, fin) => if (fin eq EmptyTree) context.warning(tree.pos, "A try without a catch or finally is equivalent to putting its body in a block; no exceptions are handled.")
          case Try(_, catches, _) => catches.foreach(checkForCatchAll)
        }
        tree
      }
      ...
      def warnMissingInterpolator(lit: Literal): Unit = if (!isPastTyper) {
        ...
        def requiresNoArgs(tp: Type): Boolean = tp match {
          ...
          case MethodType(p :: _, _) => p.isImplicit
          case _ => true
        }
      ...
      def typedSingletonTypeTree(tree: SingletonTypeTree) = {
        ...
        if (refTyped.isErrorTyped) setError(tree) else {
          tree.setType(refTyped.tpe.resultType)
          if (refTyped.isErrorTyped || treeInfo.admitsTypeSelection(refTyped)) tree else UnstableTreeError(tree)
        }
      }
      def typedSelectFromTypeTree(tree: SelectFromTypeTree) = {
        val qual1 = typedType(tree.qualifier, mode)
        if (qual1.isErrorTyped) setError(treeCopy.SelectFromTypeTree(tree, qual1, tree.name)) else if (qual1.tpe.isVolatile) TypeSelectionFromVolatileTypeError(tree, qual1) else typedSelect(tree, qual1, tree.name)
      }
      ...
      def typedExistentialTypeTree(tree: ExistentialTypeTree) = {
        val tree1 = typerWithLocalContext(context.makeNewScope(tree, context.owner))(typer => if (context.inTypeConstructorAllowed) typer.context.withinTypeConstructorAllowed(typer.typedExistentialTypeTree(tree, mode)) else typer.typedExistentialTypeTree(tree, mode))
      ...
      def typedTypTree(tree: TypTree): Tree = tree match {
        ...
        case tree: TypeTreeWithDeferredRefCheck => tree
      ...
      def typedOutsidePatternMode(tree: Tree): Tree = tree match {
        ...
        case tree: AssignOrNamedArg => typedAssign(tree.lhs, tree.rhs)
      ...
      def typedInAnyMode(tree: Tree): Tree = tree match {
        ...
        case tree: UnApply => abort(s"unexpected UnApply $tree")
        case _ => if (mode.inPatternMode) typedInPatternMode(tree) else typedOutsidePatternMode(tree)
      }
      tree match {
    ...
    def typedTypeConstructor(tree: Tree, mode: Mode): Tree = {
      ...
      result.tpe.dealias match {
        case restpe @ TypeRef(pre, _, _) if !phase.erasedTypes && !pre.isStable && !context.unit.isJava => ConstructorPrefixError(tree, restpe)
        case _ => result
      }
    }
