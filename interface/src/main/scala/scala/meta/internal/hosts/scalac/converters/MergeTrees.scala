    ...
    def loop[T <: m.Tree](pTree: T, cTree: T): T = {
      ...
      ((pTree, cTree) match {
        ...
        case (p: Term.Name, c: Term.Name) =>
          p.withDenot(c.denot).withTokens(p.tokens)
        case (p: Type.Name, c: Type.Name) =>
          p.withDenot(c.denot).withTokens(p.tokens)
        case (p: Ctor.Ref.Name, c: Ctor.Ref.Name) =>
          p.withDenot(c.denot).withTyping(c.typing).withTokens(p.tokens)
        ...
        case (p: Term.This, c: Term.This) =>
          p.copy(loop(p.qual, c.qual)).withTokens(p.tokens)
        case (p: Term.Super, c: Term.Super) =>
          p.copy(loop(p.thisp, c.thisp), loop(p.superp, c.superp)).withTokens(p.tokens)
        case (p: Term.Select, c: Term.Select) =>
          p.copy(loop(p.qual, c.qual), loop(p.name, c.name)).withTokens(p.tokens)
        case (p: Term.Interpolate, c: Term.Interpolate) =>
          p.copy(loop(p.prefix, c.prefix), zLoop(loop[Lit.String])(p.parts, c.parts), zLoop(loop[Term])(p.args, c.args)).withTokens(p.tokens)
        case (p: Term.Apply, c: Term.Apply) =>
          p.copy(loop(p.fun, c.fun), zLoop(loop[Term.Arg])(p.args, c.args)).withTokens(p.tokens)
        case (p: Term.ApplyType, c: Term.ApplyType) =>
          p.copy(loop(p.fun, c.fun), zLoop(loop[Type])(p.targs, c.targs)).withTokens(p.tokens)
        case (p: Term.ApplyInfix, c: Term.ApplyInfix) =>
          p.copy(loop(p.lhs, c.lhs), loop(p.op, c.op), zLoop(loop[Type])(p.targs, c.targs), zLoop(loop[Term.Arg])(p.args, c.args)).withTokens(p.tokens)
        case (p: Term.ApplyUnary, c: Term.ApplyUnary) =>
          p.copy(loop(p.op, c.op), loop(p.arg, c.arg)).withTokens(p.tokens)
        case (p: Term.Assign, c: Term.Assign) =>
          p.copy(loop(p.lhs, c.lhs), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Term.Update, c: Term.Update) =>
          p.copy(loop(p.fun, c.fun), zzLoop(loop[Term.Arg])(p.argss, c.argss), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Term.Return, c: Term.Return) =>
          p.copy(loop(p.expr, c.expr)).withTokens(p.tokens)
        case (p: Term.Throw, c: Term.Throw) =>
          p.copy(loop(p.expr, c.expr)).withTokens(p.tokens)
        case (p: Term.Ascribe, c: Term.Ascribe) =>
          p.copy(loop(p.expr, c.expr), loop(p.decltpe, c.decltpe)).withTokens(p.tokens)
        case (p: Term.Annotate, c: Term.Annotate) =>
          p.copy(loop(p.expr, c.expr), zLoop(loop[Mod.Annot])(p.annots, c.annots)).withTokens(p.tokens)
        case (p: Term.Tuple, c: Term.Tuple) =>
          p.copy(zLoop(loop[Term])(p.elements, c.elements)).withTokens(p.tokens)
        case (p: Term.Block, c: Term.Block) =>
          p.copy(zLoop(loop[Stat])(p.stats, c.stats)).withTokens(p.tokens)
        case (p: Term.If, c: Term.If) =>
          p.copy(loop(p.cond, c.cond), loop(p.thenp, c.thenp), loop(p.elsep, c.elsep)).withTokens(p.tokens)
        case (p: Term.Match, c: Term.Match) =>
          p.copy(loop(p.scrut, c.scrut), zLoop(loop[Case])(p.cases, c.cases)).withTokens(p.tokens)
        case (p: Term.TryWithCases, c: Term.TryWithCases) =>
          p.copy(loop(p.expr, c.expr), zLoop(loop[Case])(p.catchp, c.catchp), oLoop(loop[Term])(p.finallyp, c.finallyp)).withTokens(p.tokens)
        case (p: Term.TryWithTerm, c: Term.TryWithTerm) =>
          p.copy(loop(p.expr, c.expr), loop(p.catchp, c.catchp), oLoop(loop[Term])(p.finallyp, c.finallyp)).withTokens(p.tokens)
        case (p: Term.Function, c: Term.Function) =>
          p.copy(zLoop(loop[Term.Param])(p.params, c.params), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Term.PartialFunction, c: Term.PartialFunction) =>
          p.copy(zLoop(loop[Case])(p.cases, c.cases)).withTokens(p.tokens)
        case (p: Term.While, c: Term.While) =>
          p.copy(loop(p.expr, c.expr), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Term.Do, c: Term.Do) =>
          p.copy(loop(p.body, c.body), loop(p.expr, c.expr)).withTokens(p.tokens)
        case (p: Term.For, c: Term.For) =>
          p.copy(zLoop(loop[Enumerator])(p.enums, c.enums), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Term.ForYield, c: Term.ForYield) =>
          p.copy(zLoop(loop[Enumerator])(p.enums, c.enums), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Term.New, c: Term.New) =>
          p.copy(loop(p.templ, c.templ)).withTokens(p.tokens)
        case (p: Term.Placeholder, c: Term.Placeholder) =>
          p
        case (p: Term.Eta, c: Term.Eta) =>
          p.copy(loop(p.term, c.term)).withTokens(p.tokens)
        case (p: Term.Arg.Named, c: Term.Arg.Named) =>
          p.copy(loop(p.name, c.name), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Term.Arg.Repeated, c: Term.Arg.Repeated) =>
          p.copy(loop(p.arg, c.arg)).withTokens(p.tokens)
        case (p: Term.Param, c: Term.Param) =>
          p.copy(
            zLoop(loop[Mod])(p.mods, c.mods),
            loop(p.name, c.name),
            oLoop(loop[Type.Arg])(p.decltpe, c.decltpe),
            oLoop(loop[Term])(p.default, c.default)).withTokens(p.tokens)
        ...
        case (p: Type.Singleton, c: Type.Singleton) =>
          p.copy(loop(p.ref, c.ref)).withTokens(p.tokens)
        case (p: Type.Apply, c: m.Type.Apply) =>
          p.copy(loop(p.tpe, c.tpe), zLoop(loop[Type])(p.args, c.args)).withTokens(p.tokens)
        case (p: Type.ApplyInfix, c: Type.ApplyInfix) =>
          p.copy(loop(p.lhs, c.lhs), loop(p.op, c.op), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Type.Function, c: Type.Function) =>
          p.copy(zLoop(loop[Type.Arg])(p.params, c.params), loop(p.res, c.res)).withTokens(p.tokens)
        case (p: Type.Tuple, c: Type.Tuple) =>
          p.copy(zLoop(loop[Type])(p.elements, c.elements)).withTokens(p.tokens)
        case (p: Type.Compound, c: Type.Compound) =>
          p.copy(zLoop(loop[Type])(p.tpes, c.tpes), zLoop(loop[Stat])(p.refinement, c.refinement)).withTokens(p.tokens)
        case (p: Type.Existential, c: Type.Existential) =>
          p.copy(loop(p.tpe, c.tpe), zLoop(loop[Stat])(p.quants, c.quants)).withTokens(p.tokens)
        case (p: Type.Annotate, c: Type.Annotate) =>
          p.copy(loop(p.tpe, c.tpe), zLoop(loop[Mod.Annot])(p.annots, c.annots)).withTokens(p.tokens)
        case (p: Type.Placeholder, c: Type.Placeholder) =>
          p
        case (p: Type.Arg.ByName, c: Type.Arg.ByName) =>
          p.copy(loop(p.tpe, c.tpe)).withTokens(p.tokens)
        ...
        case (p: Type.Param, c: Type.Param) =>
          p.copy(
            ...
            zLoop(loop[Type])(p.contextBounds, c.contextBounds)).withTokens(p.tokens)
        case (p: Type.Bounds, c: Type.Bounds) =>
          p.copy(oLoop(loop[Type])(p.lo, c.lo), oLoop(loop[Type])(p.hi, c.hi)).withTokens(p.tokens)

        // Stats
        case (p: Decl.Val, c: Decl.Val) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), zLoop(loop[Pat.Var.Term])(p.pats, c.pats), loop(p.decltpe, c.decltpe)).withTokens(p.tokens)
        case (p: Decl.Var, c: Decl.Var) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), zLoop(loop[Pat.Var.Term])(p.pats, c.pats), loop(p.decltpe, c.decltpe)).withTokens(p.tokens)
        case (p: Decl.Def, c: Decl.Def) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), zzLoop(loop[Term.Param])(p.paramss, c.paramss), loop(p.decltpe, c.decltpe)).withTokens(p.tokens)
        case (p: Decl.Type, c: Decl.Type) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), loop(p.bounds, c.bounds)).withTokens(p.tokens)
        case (p: Defn.Val, c: Defn.Val) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), zLoop(loop[Pat])(p.pats, c.pats), oLoop(loop[Type])(p.decltpe, c.decltpe), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Defn.Var, c: Defn.Var) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), zLoop(loop[Pat])(p.pats, c.pats), oLoop(loop[Type])(p.decltpe, c.decltpe), oLoop(loop[Term])(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Defn.Def, c: Defn.Def) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), zzLoop(loop[Term.Param])(p.paramss, c.paramss), oLoop(loop[Type])(p.decltpe, c.decltpe), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Defn.Macro, c: Defn.Macro) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), zzLoop(loop[Term.Param])(p.paramss, c.paramss), loop(p.decltpe, c.decltpe), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Defn.Type, c: Defn.Type) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Defn.Class, c: Defn.Class) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), loop(p.ctor, c.ctor), loop(p.templ, c.templ)).withTokens(p.tokens)
        case (p: Defn.Trait, c: Defn.Trait) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zLoop(loop[Type.Param])(p.tparams, c.tparams), loop(p.ctor, c.ctor), loop(p.templ, c.templ)).withTokens(p.tokens)
        case (p: Defn.Object, c: Defn.Object) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), loop(p.ctor, c.ctor), loop(p.templ, c.templ)).withTokens(p.tokens)
        case (p: Pkg, c: Pkg) =>
          p.copy(loop(p.ref, c.ref), zLoop(loop[Stat])(p.stats, c.stats)).withTokens(p.tokens)

        // Imports
        case (p: Import, c: Import) =>
          p.copy(zLoop(loop[Import.Clause])(p.clauses, c.clauses)).withTokens(p.tokens)
        case (p: Import.Clause, c: Import.Clause) =>
          p.copy(loop(p.ref, c.ref), zLoop(loop[Import.Selector])(p.sels, c.sels)).withTokens(p.tokens)
        case (p: Import.Selector.Wildcard, c: Import.Selector.Wildcard) =>
          p
        case (p: Import.Selector.Name, c: Import.Selector.Name) =>
          p.copy(loop(p.value, c.value)).withTokens(p.tokens)
        case (p: Import.Selector.Rename, c: Import.Selector.Rename) =>
          p.copy(loop(p.from, c.from), loop(p.to, c.to)).withTokens(p.tokens)
        case (p: Import.Selector.Unimport, c: Import.Selector.Unimport) =>
          p.copy(loop(p.name, c.name)).withTokens(p.tokens)

        // Case
        case (p: Case, c: Case) =>
          p.copy(loop(p.pat, c.pat), oLoop(loop[Term])(p.cond, c.cond), loop(p.body, c.body)).withTokens(p.tokens)
        ...
        case (p: Mod.Annot, c: Mod.Annot) =>
          p.copy(loop(p.body, c.body)).withTokens(p.tokens)
        case (p: Mod.Private, c: Mod.Private) =>
          p.copy(loop(p.within, c.within)).withTokens(p.tokens)
        ...
        case (p: Pat.Tuple, c: Pat.Tuple) =>
          p.copy(zLoop(loop[Pat])(p.elements, c.elements)).withTokens(p.tokens)
        case (p: Pat.Extract, c: Pat.Extract) =>
          p.copy(loop(p.ref, c.ref), zLoop(loop[Type])(p.targs, c.targs), zLoop(loop[Pat.Arg])(p.args, c.args)).withTokens(p.tokens)
        case (p: Pat.ExtractInfix, c: Pat.ExtractInfix) =>
          p.copy(loop(p.lhs, c.lhs), loop(p.ref, c.ref), zLoop(loop[Pat.Arg])(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Pat.Interpolate, c: Pat.Interpolate) =>
          p.copy(
            loop(p.prefix, c.prefix),
            zLoop(loop[Lit.String])(p.parts, c.parts),
            zLoop(loop[Pat])(p.args, c.args)).withTokens(p.tokens)
        ...
        case (p: Pat.Type.Apply, c: Pat.Type.Apply) =>
          p.copy(loop(p.tpe, c.tpe), zLoop(loop[Pat.Type])(p.args, c.args)).withTokens(p.tokens)
        case (p: Pat.Type.ApplyInfix, c: Pat.Type.ApplyInfix) =>
          p.copy(loop(p.lhs, c.lhs), loop(p.op, c.op), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Pat.Type.Function, c: Pat.Type.Function) =>
          p.copy(zLoop(loop[Pat.Type])(p.params, c.params), loop(p.res, c.res)).withTokens(p.tokens)
        case (p: Pat.Type.Tuple, c: Pat.Type.Tuple) =>
          p.copy(zLoop(loop[Pat.Type])(p.elements, c.elements)).withTokens(p.tokens)
        case (p: Pat.Type.Compound, c: Pat.Type.Compound) =>
          p.copy(zLoop(loop[Pat.Type])(p.tpes, c.tpes), zLoop(loop[Stat])(p.refinement, c.refinement)).withTokens(p.tokens)
        ...
        case (p: Enumerator.Generator, c: Enumerator.Generator) =>
          p.copy(loop(p.pat, c.pat), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Enumerator.Val, c: Enumerator.Val) =>
          p.copy(loop(p.pat, c.pat), loop(p.rhs, c.rhs)).withTokens(p.tokens)
        case (p: Enumerator.Guard, c: Enumerator.Guard) =>
          p.copy(loop(p.cond, c.cond)).withTokens(p.tokens)
        ...
        case (p: Template, c: Template) =>
          p.copy(
            ...
            loop(p.self, c.self),
            oLoop(zLoop(loop[Stat]))(p.stats, c.stats)).withTokens(p.tokens)

        // Ctors
        case (p: Ctor.Primary, c: Ctor.Primary) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zzLoop(loop[Term.Param])(p.paramss, c.paramss)).withTokens(p.tokens)
        case (p: Ctor.Secondary, c: Ctor.Secondary) =>
          p.copy(zLoop(loop[Mod])(p.mods, c.mods), loop(p.name, c.name), zzLoop(loop[Term.Param])(p.paramss, c.paramss), loop(p.body, c.body)).withTokens(p.tokens)
        ...
        case (p @ Term.Apply(p1: Term.Select, pargs), Term.Update(c1, cargs, c2)) if cargs.size == 1 && cargs.head.size == 1 =>
          Term.Update(loop(p1.qual, c1), List(zLoop(loop[Term.Arg])(pargs.init, cargs.head)), loop(pargs.last.asInstanceOf[Term], c2)).withTokens(p.tokens)
        ...
        case (p: Type.Apply, Type.Tuple(celems)) =>
          Type.Tuple(zLoop(loop[Type])(p.args, celems)).withTokens(p.tokens)
