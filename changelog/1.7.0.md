...
It is our great pleasure to announce that the overwhelming majority of scala.meta modules are now supported by Scala.js!
In 1.7.0, it is possible to do advanced tasks that include parsing, quasiquoting and prettyprinting in a browser.
The only unsupported modules are `scalahost` and `testkit`.
...
Functionality:
- `Lit(x: Any)` got split into `Lit.Null(x: Any)`, `Lit.Int(x: Int)`, etc.
...
- If you use scalahost, you should upgrade to Scala 2.11.10 or Scala 2.12.1.
...
for precise instructions how to update your builds.
