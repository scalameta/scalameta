...
import cbc.util.Chars.isOperatorPart
...
object StdNames {
  ...
  def compactifyName(orig: String): String = compactify(orig)
  private final object compactify extends (String => String) {
    ...
    val MaxNameLength = math.min(
      settings.maxClassfileName - maxSuffixLength,
      2 * (settings.maxClassfileName - maxSuffixLength - 2*marker.length - 32)
    )
...
abstract class CommonNames {
  ...
  def flattenedName(segments: Name*): NameType =
    compactifyName(segments mkString NAME_JOIN_STRING)
