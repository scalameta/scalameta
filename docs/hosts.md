...
| `def attrs(tree: Tree): Seq[Attr]`                        | Computes and returns semantic information for a given tree: resolved, i.e. non-overloaded reference to a definition/definitions, type, inferred type and value arguments, macro expansion, etc. This API isn't fully fleshed out yet, see [semantic/package.scala](/scalameta/semantic/package.scala) for a list of currently defined attributes.
| `def root: Scope`                                         | The scope that encloses all other scopes in the program.
| `def owner(tree: Member): Scope`                          | A scope that owns the provided member, be it a definition or a statement in some definition. If the provided member has been obtained using `members` via some prefix or by instantiating some type parameters, then the results of this method should also have corresponding type parameters instantiated.
...
| `def members(scope: Scope): Seq[Member]`                  | Returns all members belonging to the specified scope. This API could query something as simple as parameters of a method or as complex as all members of a given class (accounting for inherited members and overriding). The exhaustive list of scopes that need to be handled: `Term.Block`, `Term.Function`, `Term.For`, `Term.ForYield`, `Case`, `Type`, `Member`, `Template`.<br/><br/>When called on `Type`, `Member` or `Template`, this method should return members that are adjusted to current type arguments and self type. E.g. `t"List".members` should return `Seq(q"def head: A = ...", ...)`, whereas `t"List[Int]".members` should return `Seq(q"def head: Int = ...", ...)`.
