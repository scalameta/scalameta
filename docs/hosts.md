...
| `def stats(scope: Scope): Seq[Tree]`                      | This isn't actually a method in `Host`, and it's here only to emphasize a peculiarity of our API. This particular method in unnecessary, because all lists in trees (e.g. the list of statements in a block or a list of declarations in a package or a class) are actually `Seq`'s, which means that the host can choose between eager and lazy population of scope contents when returning trees to the user of the scala.meta API (e.g. in `root`).
...
scala.meta expects hosts to signal errors by throwing exceptions derived from `scala.meta.ReflectionException`. Users of scala.meta might be shielded from these exceptions by an additional error handling layer inside scala.meta, but that shouldn't be a concern for host implementors. At the moment, we don't expose any exception hierarchy, and the only way for the host to elaborate on the details of emitted errors is passing a custom error message. This might change later.
