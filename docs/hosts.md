...
| `def symbol(tree: Tree): Symbol`                          | If `tree` is a definition, returns a symbol associated with this definition. If `tree` is a reference, returns a symbol of the underlying definition. Otherwise, returns `Symbol.Zero`. Unlike in scalac, symbols are dumb tokens that carry no semantic information. See comments in [Hygiene.scala](/scalameta/src/main/scala/scala/meta/semantic/Hygiene.scala) for more information.
| `def tree(symbol: Symbol): Tree`                          | Looks up a definition that corresponds to a given symbol.
