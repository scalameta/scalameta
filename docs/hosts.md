...
| `def parents(member: Has.Template): Seq[Has.Template]`    | Direct parents of a given class, trait or object.
| `def children(member: Has.Template): Seq[Has.Template]`   | Direct children of a given class, trait or object in the closed world reflected by the host.
| `def overrides(member: Member): Seq[Member]`              | Definitions directly overridden by a given definition in linearization order. If the provided member has been obtained via some prefix or by instantiating some type parameters, then the results of this method should also have corresponding type parameters instantiated.
| `def overriding(member: Member): Seq[Member]`             | Definitions directly overriding a given definition in the closed world reflected by the host. If the provided member has been obtained via some prefix by instantiating some type parameters, then the results of this method should also have corresponding type parameters instantiated.
