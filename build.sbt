...
lazy val sharedSettings = Def.settings(
  ...
  addCompilerPlugin("org.scalamacros" % "paradise" % "2.1.0" cross CrossVersion.fullMapped(x => if (x.startsWith("2.12.7-bin")) "2.12.6" else x)),
...
lazy val fullCrossVersionSettings = Seq(
  ...
  unmanagedSourceDirectories.in(Compile) += {
    ...
    val versionDir = if (scalaVersion.value.startsWith("2.12.7-bin")) "2.12.7" else scalaVersion.value
