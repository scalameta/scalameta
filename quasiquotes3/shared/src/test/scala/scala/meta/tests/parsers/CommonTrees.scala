...
trait CommonTrees {
  ...
  final def tplNoBody(inits: List[Init]): Template = Template(None, inits, tplBody())
  final def tplNoBody(inits: Init*): Template = tplNoBody(inits.toList)

  final def tpl(inits: List[Init], body: Template.Body): Template = Template(None, inits, body)
