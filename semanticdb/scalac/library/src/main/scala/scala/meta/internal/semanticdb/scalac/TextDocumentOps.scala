...
trait TextDocumentOps {
  ...
  implicit class XtensionCompilationUnitDocument(unit: g.CompilationUnit) {
    ...
    def toTextDocument(explicitDialect: Option[m.Dialect]): s.TextDocument = {
      ...
      def addOccurrenceFromSemantic(mpos: m.Position, ssym: String, isDef: Boolean): Unit = {
        if (isDef) binders += mpos
        occurrences(mpos) = ssym
      }
      def addOccurrence(mpos: m.Position, gsym: g.Symbol, isDef: Boolean): Unit =
        addOccurrenceFromSemantic(mpos, gsym.toSemantic, isDef)
        ...
        object traverser extends g.Traverser {
          private def trySymbolDefinition(gsym: g.Symbol): Unit = {
            ...
            if (gsym.isClass && !gsym.isTrait) {
              if (gsym.isAnonymousClass)
                addOccurrenceFromSemantic(gsym.pos.focus.toMeta, symbol, isDef = true)
          ...
          private def success(mtree: m.Name, gsym0: g.Symbol): Unit = {
            ...
            if (mtree.isDefinition) {
              ...
              } else addOccurrenceFromSemantic(pos, symbol, isDef = false)
              ...
              addOccurrenceFromSemantic(pos, symbol, isDef = false)
              ...
              case gtree: g.Literal
                ...
                gtree.value match {
                  ...
                  case g.Constant(tpe @ g.TypeRef(_, _, Nil)) =>
                    ...
                    addOccurrence(mposClazz, tpe.typeSymbol, isDef = false)
                  case _ =>
                }
