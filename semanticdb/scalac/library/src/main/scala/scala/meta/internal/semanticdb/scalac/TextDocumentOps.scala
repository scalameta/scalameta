          ...
          private def success(mtree: m.Name, gsym0: g.Symbol): Unit = {
            ...
            if (mtree.isDefinition) {
              if (!mvalpatstart.contains(pos.start))
                addOccurrenceFromSemantic(pos, symbol, Role.DEFINITION)
              else if (gsym.name.endsWith(mtree.value)) mpatoccurrences
                .update(pos, (symbol, Role.DEFINITION))
              ...
              addOccurrenceFromSemantic(pos, symbol, Role.REFERENCE)
