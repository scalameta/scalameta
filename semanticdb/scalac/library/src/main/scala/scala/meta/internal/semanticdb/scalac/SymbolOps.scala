...
trait SymbolOps {
  ...
  implicit class XtensionGSymbolMSpec(sym: g.Symbol) {
    ...
    def semanticdbDecls: SemanticdbDecls = {
      if (sym.hasPackageFlag) { SemanticdbDecls(Nil) }
        ...
        if (sym.isModuleClass && sym.isJavaDefined) { sym.companionClass.semanticdbDecls }
        else {
          @tailrec
  ...
  implicit class XtensionGSymbolsMSpec(syms: List[g.Symbol]) {
    def sscope(linkMode: LinkMode): s.Scope = {
      linkMode match {
        ...
        case HardlinkChildren => s
            .Scope(hardlinks = syms.map(_.toSymbolInformation(HardlinkChildren)))
      }
    }
  }
  ...
  implicit class XtensionGSymbol(sym: g.Symbol) {
    ...
    def isUseless: Boolean = {
      sym == g.NoSymbol || sym.isSyntheticConstructor || sym.isStaticConstructor ||
      sym.isLocalChild || sym.isSyntheticValueClassCompanion || sym.isUselessField ||
      sym.isSyntheticCaseAccessor || sym.isRefinementClass || sym.isSyntheticJavaModule
    }
    ...
    def isUselessSymbolInformation: Boolean = {
      sym.isUseless || sym.isEtaExpandedParameter || sym.isAnonymousClassConstructor ||
      sym.isSyntheticAbstractType || sym.isAnonymousSelfParameter
    }
  ...
  private def freshSymbol(sym: g.Symbol): String = {
    ...
    def loop(sym: g.Symbol): GSourceFile = {
      ...
      else { if (sym == g.NoSymbol) GNoSourceFile else loop(sym.owner) }
    }
