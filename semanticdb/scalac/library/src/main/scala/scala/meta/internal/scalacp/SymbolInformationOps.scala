...
import scala.meta.internal.semanticdb.Scala.{DisplayNames => dn, _}
...
trait SymbolInformationOps {
  ...
  implicit class XtensionGSymbolSSymbolInformation(sym: SymbolInfoSymbol) {
    ...
    private[meta] def kind: s.SymbolInformation.Kind = sym match {
      ...
      case _: ObjectSymbol | _: ClassSymbol if sym.isModule =>
        if (sym.isPackage) k.PACKAGE else if (sym.isPackageObject) k.PACKAGE_OBJECT else k.OBJECT
      ...
      case sym: ClassSymbol if !sym.isModule =>
        if (sym.isTrait && sym.isJava) k.INTERFACE else if (sym.isTrait) k.TRAIT else k.CLASS
      case _: TypeSymbol | _: AliasSymbol => if (sym.isParam) k.TYPE_PARAMETER else k.TYPE
      ...
      if (sym.isPackage) ()
        ...
        if (sym.isParam) sym.parent.foreach {
          case parent: SymbolInfoSymbol => if ((parent.properties & p.PRIMARY.value) != 0) parent
              .parent.foreach { grandParent =>
                val classMembers = grandParent.children
                val accessor = classMembers
                  .find(m => m.isParamAccessor && m.symbolName == sym.symbolName)
                accessor.foreach { accessor =>
                  val isStable = if (accessor.isMethod) accessor.isStable else !accessor.isMutable
                  if (!isStable) flip(p.VAR) else if (accessor.isMethod) flip(p.VAL) else ()
                }
              }
          case _ => ()
        }
    ...
    private def sig(linkMode: LinkMode): s.Signature =
      try
        if (sym.isPackage) s.NoSignature
        else {
          val sig =
            // NOTE: Scalap doesn't expose JAVA_ENUM.
            ...
            // } else {
            if (sym.isAlias) {
              def preprocess(tpe: Type): Type = tpe match {
                case PolyType(tpe, tparams) => PolyType(preprocess(tpe), tparams)
                case tpe => TypeBoundsType(tpe, tpe)
              }
              preprocess(sym.infoType)
            } else if (sym.isObject) sym.infoType match {
              case TypeRefType(_, moduleClassSym: SymbolInfoSymbol, _) => moduleClassSym.infoType
              case other => sys.error(s"unsupported type $other")
            }
            else sym.infoType
          val ssig = sig.toSemanticSig(linkMode)
          if (sym.isConstructor) ssig match {
            case ssig: s.MethodSignature => ssig.copy(returnType = s.NoType)
            case _ => ssig
          }
          else if (sym.isScalacField) ssig match {
    ...
    private def access: s.Access = kind match {
      ...
      case _ => sym.symbolInfo.privateWithin match {
          case None =>
          ...
          case Some(privateWithin: Symbol) =>
            ...
            if (sym.isProtected) s.ProtectedWithinAccess(ssym) else s.PrivateWithinAccess(ssym)
          case Some(other) => sys.error(s"unsupported privateWithin: ${other.getClass} $other")
        }
    }
