...
object Metacp {
  def main(args: Array[String]): Unit = {
    sys.exit(process(args, Reporter.default))
  }

  @deprecated("Use `process(Settings.parse(args.toList).get, Reporter.default)`.", "3.5.0")
  def process(args: Array[String]): Int = {
    process(args, Reporter.default)
  }
