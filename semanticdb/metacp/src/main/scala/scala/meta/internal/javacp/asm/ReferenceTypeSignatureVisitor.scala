...
class ReferenceTypeSignatureVisitor extends TypedSignatureVisitor[Option[ReferenceTypeSignature]] {
  ...
  override def result(): Option[ReferenceTypeSignature] =
    if (arrayTypeSignatureVisitor != null) arrayTypeSignatureVisitor.result() match {
  ...
  override def visitClassType(name: String): Unit = name match {
  ...
  override def visitTypeArgument(wildcard: Char): SignatureVisitor = lastSimpleClassTypeSignatures
    .visitTypeArgument(wildcard)

  override def visitTypeArgument(): Unit = lastSimpleClassTypeSignatures.visitTypeArgument()

  override def visitTypeVariable(name: String): Unit = typeVariable = TypeVariableSignature(name)
