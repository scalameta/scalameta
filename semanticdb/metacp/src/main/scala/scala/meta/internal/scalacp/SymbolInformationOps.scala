    ...
    private[meta] def properties: Int = {
      ...
      if (sym.isPackage) { () }
        ...
        if (/*kind.isLocal ||*/ sym.isUsefulField) {
          if (sym.isMutable) flip(p.VAR) else flip(p.VAL)
        }
        if (sym.isAccessor) { if (sym.isStable) flip(p.VAL) else flip(p.VAR) }
        if (sym.isParam) {
          sym.parent.foreach {
            case parent: SymbolInfoSymbol => if ((parent.properties & p.PRIMARY.value) != 0) {
                parent.parent.foreach { grandParent =>
                  ...
                  accessor.foreach { accessor =>
                    val isStable = {
                      if (accessor.isMethod) accessor.isStable else !accessor.isMutable
                    }
    ...
    private def sig(linkMode: LinkMode): s.Signature = {
      try {
        ...
        case ex: MissingSymbolException =>
          ...
          if (settings.stubBrokenSignatures) { s.NoSignature }
          else { throw ex }
