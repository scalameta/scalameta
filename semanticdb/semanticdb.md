...
Symbol occurrences are produced for names that appear in Scala source code and
resolve to a [Symbol](#scala-symbol). The `role` of an occurrence is:

- `DEFINITION` if the name introduces the definition that it resolves to, i.e.
  if it is the name of its enclosing definition. For example, `x` in
  `val x = 1` or `C` in `class C`.
- `REFERENCE` for any other name that resolves to a symbol. For example, `x`
  in `println(x)`.

The [Range](#range) of an occurrence covers the name exactly as it is written
in source, including backquotes if present. Occurrences are also produced for
several language constructs that are not spelled out as names in source, for
example primary constructors and constructor invocations. Such occurrences
have an empty range that points at the location associated with the
corresponding construct (see below).

Multiple occurrences may share the same range, each recording a separate
fact anchored to the same source construct. For example, an import selector
that refers both to a term and to a type produces two occurrences with the
same range, and an anonymous function that implements a SAM type produces
two definitions over its range (see below).

Occurrences are only produced for code that is written in source. Code
generated by the compiler, for example inferred method calls or implicit
arguments, is described by [Synthetic](#scala-synthetic) instead.

The tables below list only the occurrences that are relevant to the construct
being discussed, in the order in which they appear in the corresponding code
snippet. Local symbol names are illustrative.

**Value and variable declarations and definitions** [[39][39], [41][41]]
produce occurrences for the names that they define and for every reference to
those names:
...
<table>
  ...
  <tr>
    <td><code>x</code> in <code>val x = 1</code></td>
    <td><code>_empty_/C#x.</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>y</code> in <code>var y = 2</code></td>
    <td><code>_empty_/C#y().</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>xl</code> in <code>val xl = ...</code></td>
    <td><code>local0</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>x</code> in <code>x + y</code></td>
    <td><code>_empty_/C#x.</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>y</code> in <code>x + y</code></td>
    <td><code>_empty_/C#y().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>y</code> in <code>y = xl</code></td>
    <td><code>_empty_/C#`y_=`().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>xl</code> in <code>y = xl</code></td>
    <td><code>local0</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- Definitions and references of local values and variables resolve to
  [local symbols](#symbol).
- Reads of a member variable resolve to its getter symbol, while assignments
  to a member variable resolve to its setter symbol.
- The synthetic fields that underlie member values and variables (see
  [SymbolInformation](#scala-symbolinformation)) never produce occurrences.

**Pattern variables** [[65][65], [66][66]] produce `DEFINITION` occurrences
for the names that they bind, while extractor names in patterns are
references to the corresponding extractor:

```scala
class C {
  val Some(x) = Some(1)
  2 match { case y => println(y) }
}
```
...
<table>
  ...
  <tr>
    <td><code>Some</code> in <code>val Some(x)</code></td>
    <td><code>scala/Some.</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>x</code></td>
  ...
  <tr>
    <td><code>Some</code> in <code>Some(1)</code></td>
    <td><code>scala/Some.</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>y</code> in <code>case y</code></td>
    <td><code>local0</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>y</code> in <code>println(y)</code></td>
    <td><code>local0</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- Pattern variables bound by `val` and `var` pattern definitions in member
  position resolve to member symbols, following the same rules as regular
  value and variable definitions. All other pattern variables, including
  variables bound in `case` clauses and for comprehension enumerators,
  resolve to local symbols.
- Type variables bound by type patterns, e.g. `t` in `case _: List[t]`, also
  produce `DEFINITION` occurrences of local symbols.
- A pattern variable bound in a for comprehension enumerator produces exactly
  one `DEFINITION` occurrence, even when the compiler desugars the
  enumerator into multiple anonymous functions.

**Type declarations, type aliases and type parameters** [[42][42], [43][43]]
produce occurrences for the names that they define:
...
<table>
  ...
  <tr>
    <td><code>T</code> in <code>class C[T]</code></td>
    <td><code>_empty_/C#[T]</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>U</code> in <code>type U</code></td>
    <td><code>_empty_/C#U#</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>V</code></td>
    <td><code>_empty_/C#V#</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>Map</code></td>
    <td><code>scala/Predef.Map#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>T</code> in <code>Map[T, U]</code></td>
    <td><code>_empty_/C#[T]</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>U</code> in <code>Map[T, U]</code></td>
    <td><code>_empty_/C#U#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>

**Parameters** [\[45\]][45] produce `DEFINITION` occurrences at their
declaration site. Named arguments produce references to the corresponding
parameter of the method being invoked:

```scala
class C {
  def m[T](p: T, q: Int = 1) = p
  m(p = 2)
}
```
...
<table>
  ...
  <tr>
    <td><code>T</code></td>
    <td><code>_empty_/C#m().[T]</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>p</code> in <code>p: T</code></td>
    <td><code>_empty_/C#m().(p)</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>q</code></td>
    <td><code>_empty_/C#m().(q)</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>p</code> in <code>= p</code></td>
    <td><code>_empty_/C#m().(p)</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>p</code> in <code>m(p = 2)</code></td>
    <td><code>_empty_/C#m().(p)</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- Named arguments resolve to the parameter of the method that is actually
  invoked. For example, `name` in `User(name = "John")`, where `User` is a
  case class, resolves to `User.apply().(name)` - the parameter of the
  synthetic `apply` method of the companion object.
- Parameters of primary constructors are covered in "Constructors" below.

**Function declarations and definitions** [\[48\]][48] produce occurrences
for the names of the methods that they define and for every reference to
those names. References resolve to the exact overload, as described in
[Symbol](#scala-symbol):
...
<table>
  ...
  <tr>
    <td><code>foo</code> in <code>foo(a: Int)</code></td>
    <td><code>_empty_/M.foo().</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>foo</code> in <code>foo(a: String)</code></td>
    <td><code>_empty_/M.foo(+1).</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>foo</code> in <code>foo(1)</code></td>
    <td><code>_empty_/M.foo().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>foo</code> in <code>foo("")</code></td>
    <td><code>_empty_/M.foo(+1).</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>+</code></td>
    <td><code>scala/Int#`+`(+4).</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- Ranges of occurrences of symbolic names cover the operator token, and
  ranges of occurrences of backquoted names include the backquotes.

**Constructors** [\[53\]][53] produce occurrences both for definitions and
for invocations, even though constructors are not spelled out as names in
source. Such occurrences have an empty range:
...
<table>
  ...
  <tr>
    <td>Primary constructor (empty range after <code>C</code>)</td>
    <td><code>_empty_/C#`&lt;init&gt;`().</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><code>this</code> in <code>def this()</code></td>
    <td><code>_empty_/C#`&lt;init&gt;`(+1).</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td>Self invocation <code>this(42)</code> (empty range)</td>
    <td><code>_empty_/C#`&lt;init&gt;`().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>C</code> in <code>new C(43)</code></td>
    <td><code>_empty_/C#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td>Invocation in <code>new C(43)</code> (empty range after <code>C</code>)</td>
    <td><code>_empty_/C#`&lt;init&gt;`().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- The empty range of a primary constructor definition points right after the
  class name. Due to implementation restrictions, this occurrence is not
  produced when the class has neither an explicit parameter clause nor a
  template body, e.g. `class C`.
- Secondary constructor definitions produce an occurrence whose range covers
  the `this` keyword in `def this(...)`.
- At its definition site, a primary constructor parameter resolves to the
  symbol of the class member that it gives rise to - the value symbol for
  plain and `val` parameters and the getter symbol for `var` parameters. The
  associated constructor parameter symbol, e.g.
  <code>_empty_/C#`&lt;init&gt;`().(x)</code>, does not produce an
  occurrence.
- Parent constructor invocations in extends clauses also produce empty-range
  references right after the name of the parent type. For example,
  `class D extends C(1)` produces a reference to
  <code>_empty_/C#`&lt;init&gt;`().</code> right after `C`.

**Class, trait and object definitions** [[52][52], [58][58], [59][59]]
produce occurrences for the names that they define, for the parents that
they reference and for their self parameters:

```scala
trait B
class C extends B { self =>
}
object M {
  val anon = new B {}
}
...
<table>
  ...
  <tr>
    <td><code>B</code> in <code>trait B</code></td>
    <td><code>_empty_/B#</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>C</code></td>
  ...
  <tr>
    <td><code>B</code> in <code>extends B</code></td>
    <td><code>_empty_/B#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
  ...
  <tr>
    <td><code>M</code></td>
    <td><code>_empty_/M.</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td>Anonymous class (empty range before <code>B</code>)</td>
    <td><code>local1</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>B</code> in <code>new B {}</code></td>
    <td><code>_empty_/B#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- Named self parameters produce a `DEFINITION` occurrence of a local symbol.
  Self parameters declared as `_` or `this` produce no occurrence, but their
  type annotations still produce references.
- Anonymous classes produce an empty-range `DEFINITION` occurrence of a local
  symbol at the start of the instantiated parent, in addition to the
  reference to the parent and an empty-range reference to the invoked
  constructor (`java/lang/Object#`&lt;init&gt;`().` when the parent is a
  trait).

**Imports** [\[100\]][100] produce references for every name in an import
path:
...
<table>
  ...
  <tr>
    ...
    <td><code>scala/concurrent/</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><code>scala/util/</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>Try</code></td>
    <td><code>scala/util/Try.</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>Try</code></td>
...
- An import selector produces one occurrence for every member of the
  qualifier that has the imported name. In the example above, `Future`
  resolves both to an object and to a class, producing two occurrences with
  the same range.
- In a rename `{Try => T}`, only the original name produces occurrences. Due
  to implementation restrictions, the new name `T` produces no occurrence.
  References to the renamed name elsewhere in the code resolve directly to
  the renamed symbol.
- Wildcard selectors produce no occurrences.

**Packages and package objects** [[61][61], [60][60]] produce occurrences
for the names in package clauses and package object definitions:
...
<table>
  ...
  <tr>
    <td><code>p</code></td>
    <td><code>example/p/package.</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>x</code></td>
    <td><code>example/p/package.x.</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
</table>
...
- In a multi-segment package clause, e.g. `package a.b.c`, every segment
  resolves to the corresponding package symbol (`a/`, `a/b/` and `a/b/c/`).
  Due to implementation restrictions, all segments, including the last one,
  currently have role `REFERENCE`.
- Package names in qualified references elsewhere in the code, e.g.
  `scala.util.Either`, produce `REFERENCE` occurrences of the corresponding
  package symbols.
- References to members of package objects resolve to symbols owned by the
  package object, e.g. `classTag` in `scala.reflect.classTag[Int]` resolves
  to `scala/reflect/package.classTag().`.

**`this`, `super` and access qualifiers** [[62][62], [63][63], [101][101]]
produce references to the enclosing definitions and parents that they
resolve to:

```scala
trait M { def m: Int = 1 }
class C extends M {
  private[C] def n = C.this.m + super[M].m
}
```
...
<table>
  ...
  <tr>
    <td><code>C</code> in <code>private[C]</code></td>
    <td><code>_empty_/C#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>C</code> in <code>C.this</code></td>
    <td><code>_empty_/C#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>m</code> in <code>C.this.m</code></td>
    <td><code>_empty_/M#m().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>M</code> in <code>super[M]</code></td>
    <td><code>_empty_/M#</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
  <tr>
    <td><code>m</code> in <code>super[M].m</code></td>
    <td><code>_empty_/M#m().</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- The qualifier of an access modifier resolves to the class or package that
  it names, e.g. `example` in `protected[example]` resolves to `example/`.
- A bare `this` expression produces no occurrence. A `super` expression
  without an explicit prefix, including `super[M]`, produces an empty-range
  reference for its implicit `this` qualifier, resolving to the enclosing
  definition. The referenced parent produces an empty-range occurrence only
  when it is not written in source, i.e. in `super.m` but not in
  `super[M].m`, where the reference covers `M`.

**Anonymous functions** [\[102\]][102] produce occurrences for their
parameters. Anonymous functions that implement a SAM (single abstract
method) type additionally produce definitions that cover the whole function:

```scala
class C {
  trait SAM { def foo(a: Int): Int }
  def withSAM(f: SAM) = f.foo(0)
  withSAM(_ + 1)
  List(1).map(i => i + 1)
}
```
...
<table>
  ...
  <tr>
    <td><code>i</code> in <code>i =&gt; i + 1</code></td>
    <td><code>local2</code></td>
    <td><code>DEFINITION</code></td>
  </tr>
  <tr>
    <td><code>i</code> in <code>i + 1</code></td>
    <td><code>local2</code></td>
    <td><code>REFERENCE</code></td>
  </tr>
</table>
...
- An anonymous function or eta-expanded method value in SAM position
  produces two `DEFINITION` occurrences of local symbols whose range covers
  the entire anonymous function, or the name of the eta-expanded method: one
  for the anonymous class that implements the SAM type and one for the
  method that implements the single abstract method. An eta-expanded method
  value additionally produces a regular reference to the expanded method
  with the same range.
- Anonymous functions of standard `FunctionN` types produce occurrences only
  for their parameters and bodies.

**Other constructs.** The following constructs are subject to special rules,
most of them due to implementation restrictions:

- Member access on [structural types](#scala-type) produces a
  `REFERENCE` occurrence of a local symbol if the structural type is declared
  in the same file. If the structural type is declared in another file, no
  occurrence is produced.
- In `classOf[T]`, the `classOf` name resolves to `scala/Predef.classOf().`.
  The type argument produces a single reference covering the whole type
  expression if it is a simple, possibly qualified, type reference, e.g.
  `Int`, `java.lang.String` or `T#C`. Applied type arguments, e.g.
  `Option[Int]`, produce no occurrence.
- Names inserted or rewritten by the compiler produce no occurrences. This
  includes implicit conversions and implicit arguments (described by
  [Synthetic](#scala-synthetic) instead), as well as numeric widening, e.g.
  the implied `.toLong` in `val x: Long = 1.to(2).length`.
- Synthetic members produce no definition occurrences, because no name in
  source defines them. This includes synthetic case class methods such as
  `apply` and `copy`, synthetic constructors of objects and traits, and
  synthetic fields. A case class parameter is not synthetic in this sense:
  `x` in `case class C(x: Int)` produces a regular definition occurrence of
  the accessor `_empty_/C#x.`, as described in "Constructors" above.
  Explicit source references to synthetic members are regular references,
  e.g. `copy` in `u.copy(age = 43)` resolves to the synthetic `copy` method
  of the case class. Only compiler-inserted references, e.g. the `apply`
  implied by `User("John")`, produce no occurrences and are described by
  [Synthetic](#scala-synthetic) instead.
- For definitions transformed by macro annotations, occurrences are computed
  from the original source, before macro expansion. Due to implementation
  restrictions, some occurrences may be missing.
...
[100]: https://www.scala-lang.org/files/archive/spec/2.12/04-basic-declarations-and-definitions.html#import-clauses
[101]: https://www.scala-lang.org/files/archive/spec/2.12/06-expressions.html#this-and-super
[102]: https://www.scala-lang.org/files/archive/spec/2.12/06-expressions.html#anonymous-functions
