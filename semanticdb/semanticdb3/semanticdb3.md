...
Here is an exhaustive list of the kinds of synthetics that are generated by the
scalac plugin, along with examples:
...
```scala
List => *.unapplySeq[Int]
  unapplySeq => scala/collection/generic/SeqFactory#unapplySeq().
  Int => scala/Int#
```
...
```scala
List(1).map(_ + 2) => *(canBuildFrom[Int])
