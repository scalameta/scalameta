...
import scala.reflect.syntactic.parser.ParserInfo._
import scala.{Seq => _}
import scala.collection.immutable.Seq

// TODO: what about Print[Member] and likes?
// TODO: spelling out all these instances amounts for a lot of boilerplate. how about deriving it?
// TODO: also having independent instances makes it very difficult to communicate stuff like indentation

object Printers {
  // Term
  implicit val printTermName: Print[Term.Name] = Print { t => printName(t) }
  ...
  implicit val printTypeName: Print[Type.Name] = Print { t => printName(t) }
  ...
  // Pkg
  implicit val printPkgRoot: Print[Pkg.Root] = Print { t =>
    p("package root { ", r(t.stats, "; "), " }")
  }
  ...
  implicit val printName: Print[Name] = Print { t =>
  ...
  implicit val printParam: Print[Param] = Print {
    case t: Param.Anonymous => p(t)
    case t: Param.Named => p(t)
  }
  implicit val printAnonymousParam: Print[Param.Anonymous] = Print { t =>
    p(t.mods, "_", t.decltpe)
  }
  implicit val printNamedParam: Print[Param.Named] = Print { t =>
  ...
  implicit val printTypeParam: Print[TypeParam] = Print {
    case t: TypeParam.Anonymous => p(t)
    case t: TypeParam.Named => p(t)
  }
  implicit val printAnonymousTypeParam: Print[TypeParam.Anonymous] = Print { t =>
    val cbounds = r(t.contextBounds.map { p(": ", _) }, "")
    val vbounds = r(t.contextBounds.map { p("<% ", _) }, "")
    p(t.mods, "_", t.tparams, cbounds, vbounds, t.bounds)
  }
  implicit val printNamedTypeParam: Print[TypeParam.Named] = Print { t =>
    val cbounds = r(t.contextBounds.map { p(": ", _) }, "")
    val vbounds = r(t.contextBounds.map { p("<% ", _) }, "")
  ...
  implicit val printNamedParamLists: Print[(Seq[Seq[Param.Named]], Seq[Param.Named])] = Print { case (expl, impl) =>
    p(r(expl, ""),
      if (impl.isEmpty) ""
      else p("(implicit ", r(impl, ", "), ")"))
  }
  implicit val printAnonymousParamLists: Print[(Seq[Seq[Param.Anonymous]], Seq[Param.Anonymous])] = Print { case (expl, impl) =>
    p(r(expl, ""),
      if (impl.isEmpty) ""
      else p("(implicit ", r(impl, ", "), ")"))
  }
