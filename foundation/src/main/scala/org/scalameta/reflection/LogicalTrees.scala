  ...
  trait LogicalTrees { l =>
    ...
    object Template {
      ...
      def unapply(tree: g.Template): Some[(List[g.Tree], List[g.Tree], g.ValDef, List[g.Tree])] = {
        def indexOfFirstCtor(trees: List[g.Tree]) = trees.indexWhere { case LowlevelCtor(_, _) => true; case _ => false }
        object LowlevelCtor {
          def unapply(tree: g.Tree): Option[(List[List[g.ValDef]], List[g.Tree])] = tree match {
            case g.DefDef(_, nme.MIXIN_CONSTRUCTOR, _, _, _, SyntacticBlock(lvdefs :+ _)) =>
              Some((Nil, lvdefs))
            case g.DefDef(_, nme.CONSTRUCTOR, Nil, vparamss, _, SyntacticBlock(lvdefs :+ _ :+ _)) =>
        ...
        val lself = if (self != noSelfType) self else g.ValDef(g.Modifiers(), g.nme.WILDCARD, g.TypeTree(), g.EmptyTree).setParent(tree)
        ...
        Some((edefs, parents.map(_.appendMetadata("isParentClause" -> true)), lself, lstats))
      }
    }
    ...
    object Parent {
      ...
      def unapply(tree: g.Tree): Option[(g.Tree, l.CtorIdent, List[List[g.Tree]])] = {
        tree.parent match {
          case g.Template(_, _, stats) if tree.hasMetadata("isParentClause") =>
            val applied = dissectApplied(tree)
            (applied.callee, applied.core, applied.argss) match {
              case (tpt, classRef: g.RefTree, syntacticArgss) =>
                val semanticSupercall = stats.collectFirst{ case l.CtorDef(_, _, _, _, superCall, _) => superCall }
                semanticSupercall.map(semanticSupercall => {
                  val isSemantic = semanticSupercall.symbol != g.NoSymbol
                  val applied = dissectApplied(semanticSupercall)
                  val (semanticCtorRef: g.RefTree, semanticArgss) = (applied.callee, applied.argss)
                  val ctor = l.CtorIdent(semanticCtorRef, classRef).setParent(tree.parent)
                  val argss = if (isSemantic) semanticArgss else syntacticArgss
                  (tpt.removeMetadata("isParentClause"), ctor, argss)
                })
              case _ =>
                None
            }
    ...
    object ParamDef {
      ...
      def unapply(tree: g.ValDef): Option[(List[l.Modifier], l.TermName, g.Tree, g.Tree)] = {
        ...
        if (tree == noSelfType || tree.name.startsWith(nme.EVIDENCE_PARAM_PREFIX)) return None
    ...
    object DefDef {
      ...
      def unapply(tree: g.DefDef): Option[(List[l.Modifier], l.TermName, List[g.TypeDef], List[List[g.ValDef]], g.Tree, g.Tree)] = {
        ...
        if (rhs.isEmpty || tree.mods.hasFlag(MACRO)) return None
    ...
    object CtorIdent {
      def apply(ctorRef: g.RefTree, classRef: g.RefTree): l.CtorIdent = {
        require(ctorRef.name == nme.CONSTRUCTOR)
        ...
        val lpre = ctorRef.symbol.owner.prefix
        val lsym = ctorRef.symbol
        val lname = l.CtorName(l.Denotation(lpre, lsym), classRef.displayName)
    ...
    case class Private(within: l.Name) extends Modifier
    case class Protected(within: l.Name) extends Modifier
      ...
      def apply(tree: g.MemberDef): List[l.Modifier] = {
        ...
        val laccessQualifierMods: List[l.Modifier] = {
          ...
          if (mods.hasFlag(SYNTHETIC) && mods.hasFlag(ARTIFACT)) {
            ...
            val lprivateWithin = l.ThisName(privateWithinSym)
        ...
        if (cpinfo.nonEmpty) result.filter({ case l.Private(l.ThisName(_)) => false; case _ => true })
        else result
      }
    }
    ...
    case class AnonymousName(sym: g.Symbol) extends Name with TermParamName with TypeParamName { def denot = l.Denotation(sym.prefix, sym) }
    case class ThisName(sym: g.Symbol) extends Name { def denot = l.Denotation(sym.prefix, sym) }
    ...
    case class IndeterminateName(denot: l.Denotation, value: String) extends Name
